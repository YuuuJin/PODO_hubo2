...
int PushCANMessage(MANUAL_CAN MCData)
{
    ...
    std::cout << "Fail to send Manual CAN..!!" << std::endl; //fir simulation
    return 1;
}
...
int MCJointGainOverride_old(unsigned int _canch, unsigned int _bno, int _gain1, int _gain2, int _msec){
    ...
    MCData.data[0] = _bno;
    MCData.data[1] = 0x71;
    MCData.data[2] = (unsigned char)(_gain1 & 0x00FF);
    MCData.data[3] = (unsigned char)(_gain2 & 0x00FF);
    MCData.data[4] = (unsigned char)(_msec & 0x00FF);
    MCData.data[5] = (unsigned char)((_msec>>8) & 0x00FF);
    MCData.data[6] = (unsigned char)((_gain1>>8) & 0x00FF);
    MCData.data[7] = (unsigned char)((_gain2>>8) & 0x00FF);
