...
void HB_PreviewWalk::DampingControl2(){ //  Only FT used
    ...
    // y -direction
    double Ks =2430.9;  //after new leg part and torso mass
...
void HB_PreviewWalk::DampingControl3(){  // FT and IMU are used
    ...
    double Ks = 3000;//2245.4;//2430.9; //2245.4
    double C = 45.71;//1.780; // 38.71
    double m = 40;

    double Ksx = 1000;//879.9401;
    double Cx = 34.3596;
...
//    vec3 Kc = vec3(-0.4440,    0.0301, 0); // pole placement [-6+0.001i -6-0.001i];
    ...
    mat3 Ke = mat3(9.2710 ,   0.0714,   -0.0667,
...
//    dCOM_error_local = global2local_vec(qRF_ref, qLF_ref, dCOM_m_diff - dCOM_LIPM);  // dCOM error local
    ...
    COM_error_local = vec3(0,0,0);// global2local_vec(qRF_ref, qLF_ref, COM_e_imu_local_filtered);     // COM error local
    dCOM_error_local = vec3(0,0,0);// global2local_vec(qRF_ref, qLF_ref, dCOM_e_imu_local_filtered);  // dCOM error local
    ...
    if(step_phase > N_step ){
        ZMP_error_global = alpha_zmp_e*ZMP_error_global + (1 - alpha_zmp_e)*(ZMP_global - (pRF_ref + pLF_ref)/2);

        ZMP_error_local = global2local_vec(qRF_ref, qLF_ref, ZMP_error_global); // ZMP error local

        dY_obs = A_obs*Y_obs + B_obs*(-dot(Kc,Y_obs)) + Ke*(vec3(ZMP_error_local.y,0,0) - (C_obs*Y_obs + D_obs*(-dot(Kc,Y_obs))))- Y_obs/0.5;
    }
    Y_obs = Y_obs + dY_obs*dt;
...
vec4 HB_PreviewWalk::Ankle_Torque_from_cZMP(vec3 _global_cZMP, vec3 _global_ZMP, quat _global_qRF, quat _global_qLF,
    ...
    vec3 LF_lt1 = vec3((FootLength+_Xoffset)/2+0.02, (FootWidth+_Yoffset)/2+0.01, 0);
    vec3 LF_rt2 = vec3((FootLength+_Xoffset)/2+0.02, -(FootWidth+_Yoffset)/2+0.01, 0);
    vec3 LF_rb3 = vec3(-(FootLength+_Xoffset)/2+0.02, -(FootWidth+_Yoffset)/2+0.01, 0);
    vec3 LF_lb4 = vec3(-(FootLength+_Xoffset)/2+0.02, (FootWidth+_Yoffset)/2+0.01, 0);

    vec3 RF_lt1 = vec3((FootLength+_Xoffset)/2+0.02, (FootWidth+_Yoffset)/2-0.01, 0);
    vec3 RF_rt2 = vec3((FootLength+_Xoffset)/2+0.02, -(FootWidth+_Yoffset)/2-0.01, 0);
    vec3 RF_rb3 = vec3(-(FootLength+_Xoffset)/2+0.02, -(FootWidth+_Yoffset)/2-0.01, 0);
    vec3 RF_lb4 = vec3(-(FootLength+_Xoffset)/2+0.02, (FootWidth+_Yoffset)/2-0.01, 0);
...
void HB_PreviewWalk::AnkleTorqueController_pos(double _RAR_T_ref, double _RAP_T_ref, double _LAR_T_ref, double _LAP_T_ref,
    ...
    //// variable gains
    double gain_roll_max = -0.1;//-0.5;
    double gain_roll_min = -0.02;//-0.045;//-0.045;

    double gain_pitch_max = -0.1;//-0.5;//-0.5;
    double gain_pitch_min = -0.02;//-0.06;//-0.045;

    if(step_phase <= 1 || step_phase > N_step - 2){
        gain_roll_max = -0.05;//-0.5;
        gain_roll_min = -0.005;
        gain_pitch_max = -0.05;//-0.5;
        gain_pitch_min = -0.005;
    ...
    if(Standing_mode_flag == true){
        ...
        gain_roll_min = -0.0015;
        gain_pitch_max = -0.01;//-0.5;
        gain_pitch_min = -0.003;
    }
...
//    double decrease_factor = 3.0;
    ...
    //// filterring
    double alpha_F = 1/(1 + 2*PI*dt*100);
    ...
    double alpha_ref = 1/(1 + 2*PI*dt*2.5);
...
//    RF_variable_gain_pitch = gain_pitch_air*1/(1 + _F_RF_filtered_new.z/decrease_factor_pitch);
    ...
    RF_variable_gain_roll = (gain_roll_min - gain_roll_max)/200.0*_F_RF_filtered_new.z + gain_roll_max;
    RF_variable_gain_pitch = (gain_pitch_min - gain_pitch_max)/200.0*_F_RF_filtered_new.z + gain_pitch_max;
    ...
    RF_Arial_return_factor = 0.15;//0.1*(10 + _F_RF_filtered.z)/10;
...
//    //cout<<"roll gain : "<<LF_varible_gain_roll<<"  pitch gain : "<<LF_variable_gain_pitch<<endl;
    ...
    LF_variable_gain_roll = (gain_roll_min - gain_roll_max)/200.0*_F_LF_filtered_new.z + gain_roll_max;
    LF_variable_gain_pitch = (gain_pitch_min - gain_pitch_max)/200.0*_F_LF_filtered_new.z + gain_pitch_max;
    ...
    LF_Arial_return_factor = 0.15;//0.1*(10 + _F_LF_filtered.z)/10;
...
void HB_PreviewWalk::DSP_Fz_controller3(char _swingFoot, double _F_RF_z, double _F_LF_z, vec3 _IMU_anlge_rad){
...
//        DSP_time_flag = false;
    ...
    //get alpha new
    Alpha_dsp = fabs(cZMP_dsp_local_proj.y - local_pLF.y)/fabs(local_pLF.y - local_pRF.y);


    if(Alpha_dsp > 1.0) Alpha_dsp = 1.0;
    if(Alpha_dsp < 0) Alpha_dsp = 0;

    Alpha_dsp_before_saturate = Alpha_dsp;


    if(Alpha_dsp < 0.2)Alpha_dsp = 0.2;
    if(Alpha_dsp > 0.8)Alpha_dsp = 0.8;

    double alpha_dsp = 1/(1 + 2*PI*dt*100.0);
    Alpha_dsp_filtered = alpha_dsp*Alpha_dsp_filtered + (1.0-alpha_dsp)*Alpha_dsp;
    ...
    if(step_phase <= 1 ){
        ...
        dsp_tilt_gain = 2.5;//1.5;//0.9

    }
    else if(step_phase > N_step-1){
         dsp_ctrl_gain = 0.0001; //0.0008
        dsp_tilt_gain = 2.0;//1.5;//0.9
    }
    else{
        dsp_ctrl_gain = 0.0001; //0.0012
        dsp_tilt_gain = 2.0;//6;//3.0;//6.0; //18
    }
    ...
    RF_Fz_ref = Alpha_dsp_filtered*Fz_total;
    LF_Fz_ref = (1.0 - Alpha_dsp_filtered)*Fz_total;
...
//    dsp_tilt_gain = 0;
    ...
    if(DSP_time_flag == true || DSP_force_flag == true){
        ...
        dz_ctrl = dsp_ctrl_gain/2*Fz_diff_error - z_ctrl/0.15;



        if(dz_ctrl > 0.2) dz_ctrl = 0.2;
        if(dz_ctrl < -0.2) dz_ctrl = -0.2;
...
//    Pelv_roll_vel_ref = -2.0*_IMU_anlge_rad.x;// - Pelv_roll_ref/0.5;
//    Pelv_pitch_vel_ref = -2.0*_IMU_anlge_rad.y;// - Pelv_pitch_ref/0.5;


//    Pelv_roll_ref += Pelv_roll_vel_ref*dt; // in rad
//    Pelv_pitch_ref += Pelv_pitch_vel_ref*dt; // in rad
    ...
    double alpha_dzdz = 1/(1 + 2*PI*dt*100);
    z_ctrl_filtered = alpha_dzdz*z_ctrl_filtered + (1.0-alpha_dzdz)*z_ctrl;


//    // torso control

//    if(Pelv_roll_ref >= 3*D2R)  Pelv_roll_ref = 3*D2R;
//    if(Pelv_roll_ref <= -3*D2R) Pelv_roll_ref = -3*D2R;

//    if(Pelv_pitch_ref >= 3*D2R)  Pelv_pitch_ref = 3*D2R;
//    if(Pelv_pitch_ref <= -3*D2R) Pelv_pitch_ref = -3*D2R;
...
//        DSP_time_flag = false;
    ...
    vec3 LF_lt1 = vec3(FootLength/2+0.02, FootWidth/2+0.01, 0);
    vec3 LF_rt2 = vec3(FootLength/2+0.02, -FootWidth/2+0.01, 0);
    vec3 LF_rb3 = vec3(-FootLength/2+0.02, -FootWidth/2+0.01, 0);
    vec3 LF_lb4 = vec3(-FootLength/2+0.02, FootWidth/2+0.01, 0);

    vec3 RF_lt1 = vec3(FootLength/2+0.02, FootWidth/2-0.01, 0);
    vec3 RF_rt2 = vec3(FootLength/2+0.02, -FootWidth/2-0.01, 0);
    vec3 RF_rb3 = vec3(-FootLength/2+0.02, -FootWidth/2-0.01, 0);
    vec3 RF_lb4 = vec3(-FootLength/2+0.02, FootWidth/2-0.01, 0);
...
//        // get Alpha
//        vec3 n = vec3(local_pLF.x - local_pRF.x, (local_pLF.y - FootWidth/2+0.01) - (local_pRF.y + FootWidth/2-0.01), 0);
//        vec3 p = vec3(cZMP_dsp_local_proj.x - local_pRF.x, cZMP_dsp_local_proj.y - (local_pRF.y + FootWidth/2-0.01), 0);

//        vec3 p2n_proj = n*(dot(p,n)/dot(n));

//        vec3 P_alpha = vec3(local_pRF.x + p2n_proj.x, (local_pRF.y + FootWidth/2-0.01) + p2n_proj.y,0);

//        Alpha_dsp = sqrt(distSq({local_pLF.x, (local_pLF.y - FootWidth/2+0.01)},{P_alpha.x, P_alpha.y}))/sqrt(distSq({local_pLF.x, (local_pLF.y - FootWidth/2+0.01)},{local_pRF.x, (local_pRF.y + FootWidth/2-0.01)}));
        // get Alpha
        vec3 n = vec3(local_pLF.x - local_pRF.x, (local_pLF.y) - (local_pRF.y), 0);
        vec3 p = vec3(cZMP_dsp_local_proj.x - local_pRF.x, cZMP_dsp_local_proj.y - (local_pRF.y), 0);

        vec3 p2n_proj = n*(dot(p,n)/dot(n));

        vec3 P_alpha = vec3(local_pRF.x + p2n_proj.x, (local_pRF.y ) + p2n_proj.y,0);

        Alpha_dsp = sqrt(distSq({local_pLF.x, (local_pLF.y)},{P_alpha.x, P_alpha.y}))/sqrt(distSq({local_pLF.x, (local_pLF.y )},{local_pRF.x, (local_pRF.y)}));
    }
    ...
    if(Alpha_dsp < 0.1)Alpha_dsp = 0.1;
    if(Alpha_dsp > 0.9)Alpha_dsp = 0.9;
...
//    Alpha_dsp = (A + 1.0)/2.0;
    ...
    if(step_phase <= 1 || step_phase > N_step - 2){
        dsp_ctrl_gain = 0.0001; //0.0008
        dsp_tilt_gain = 5.0;//0.9
    }
    else{
        dsp_ctrl_gain = 0.00012; //0.0012
        dsp_tilt_gain = 3;//6;//3.0;//6.0; //18
    }
...
//                    +0*L*dsp_tilt_gain*COM_e_imu_local.x - z_ctrl/0.3;
        ...
        dz_ctrl = dsp_tilt_gain/10.0*COM_e_imu_local.y
                    - z_ctrl/0.3;
...
void HB_PreviewWalk::Calc_del_pos_from_Joystick(int _RJOG_RL, int _RJOG_UD, int _LJOG_RL, int _LJOG_UD, double _des_t_step){
    ...
    //version 1
    double del_pos_Max_x = 0.22;
    double del_pos_min_x = 0.20;

    double del_pos_Max_y = 0.10;
...
vec3 HB_PreviewWalk::StancF_to_NextF_limitation(char _swingFoot, vec3 _stancF_to_NextF){
    ...
    if(_swingFoot == LFoot){
        if(_stancF_to_NextF.y <= 0.17) _stancF_to_NextF.y = 0.17;
        if(_stancF_to_NextF.y >= 0.3) _stancF_to_NextF.y = 0.3;
    }
    if(_swingFoot == RFoot){
        if(_stancF_to_NextF.y >= -0.17) _stancF_to_NextF.y = -0.17;
        if(_stancF_to_NextF.y <= -0.3) _stancF_to_NextF.y = -0.3;
...
//    if(Alpha_landing <= 0.0) Alpha_landing = 0.0;
    ...
    double Fz_total = 400.0; //32kg
    double RF_Threshold = Alpha_dsp_before_saturate*Fz_total;
    double LF_Threshold = (1.0 - Alpha_dsp_before_saturate)*Fz_total;

    if(RF_Threshold < 25.0) RF_Threshold = 25.0;
    if(LF_Threshold < 25.0) LF_Threshold = 25.0;
