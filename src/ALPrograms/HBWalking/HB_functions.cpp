...
{
    userData->M2G.DRILL_Data[2] = 0;
    Point p = S.top();
    userData->M2G.DRILL_Data[2] = 1;
    userData->M2G.DRILL_Data[3] = S.size();
    S.pop();
    userData->M2G.DRILL_Data[2] = 2;
    Point res = S.top();
    userData->M2G.DRILL_Data[2] = 3;
    S.push(p);
    userData->M2G.DRILL_Data[2] = 4;
    return res;
}
...
{
    for(int i=0 ; i<n ; i++){
         userData->M2G.pointsx[i] = points[i].x;
         userData->M2G.pointsy[i] = points[i].y;
         userData->M2G.DRILL_Data[5] = n;
    }

   userData->M2G.valveMode = 7000;
   ...
   for (int i = 3; i < m; i++)
   {
      ...
      // a non-left turn
       userData->M2G.DRILL_Data[0] = i;
       userData->M2G.DRILL_Data[1] = m;
...
//         S.pop();
       ...
       int temp_cnt = 0;
       while (1){
           userData->M2G.DRILL_Data[6] = temp_cnt;
           int ori_info = orientation(nextToTop(S), S.top(), points[i]);
           if(ori_info == 2)
              break;
           else if(ori_info == 0){
                ori_zero = true;
...
vec3 zmpProjectionTo_StanceFoot(int _swingFoot, vec3 _Vector_stanceFootFrame, double _Xoffset, double _Yoffset){
    ...
    if(_swingFoot == -1){ // RFoot swing --> LF stance
        ...
        if(Vector_Saturated_.x < -((FootLength+_Xoffset)/2-0.03)) Vector_Saturated_.x = -((FootLength+_Xoffset)/2-0.03);
...
//        if(Vector_Saturated_.y < -(FootWidth+_Yoffset)/2) Vector_Saturated_.y = -(FootWidth+_Yoffset)/2;
        if(Vector_Saturated_.y > 0.033) Vector_Saturated_.y = 0.033;
        if(Vector_Saturated_.y < -0.04) Vector_Saturated_.y = -0.04;
    }
