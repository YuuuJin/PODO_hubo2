...
//    pRBCORE_SHM_SENSOR      sharedSEN;
    ...
    double Alpha_dsp, Alpha_AT, Alpha_dsp_filtered, Alpha_dsp_before_saturate;
    void DSP_Fz_controller(char _swingFoot, double _F_RF_z, double _F_LF_z);
    void DSP_Fz_controller2(char _swingFoot);
    void DSP_Fz_controller3(char _swingFoot, double _F_RF_z, double _F_LF_z, vec3 _IMU_anlge_rad);
    ...
    double RSP_des_deg, LSP_des_deg;
    ...
    // MemberFunctions
    int Preveiw_walking();
    void MeasurementInput(RState _RS);
    ...
    HB_PreviewWalk(){
        ...
        HipTorqueControl_flag = false;

        Landing_angle_control_flag = true;
        ...
        Swing_leg_vibration_control_flag = false;
...
//        LF_landing_angle = rpy(0,0,0);
        ...
        for(int i = 0; i < N_step + 3 ;i++){
            if(i == 0){
                ...
                SD_temp.Fpos = zmp_ini;// + vec3(0,+0.1*R_or_L,0); //for fast start
                SD_temp.Fquat = quat();
                SD_temp.yaw_rad = 0;
...
//                SDB.push_back(SD_temp);
    ...
    int PreviewGainLoad(double _zc){
        ...
        ifstream file;
        file.open("../PreviewGain.txt",ios::in);
