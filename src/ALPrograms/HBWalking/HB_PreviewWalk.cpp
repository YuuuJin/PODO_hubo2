...
int HB_PreviewWalk::Preveiw_walking(){
    ...
    if(step_phase_change_flag == true){
       //// -------------------------------joystick mode-----------------------------------------
        ...
        if(Joystick_walk_flag == true && step_phase >= 1){

            cout<<"delpos.x: "<<del_pos.x<<endl;
...
//                SD_last.Fpos.x = SDB[step_phase+1].Fpos.x;

                if(SDB[step_phase].swingFoot == RFoot){
                    rpy Euler_foot = rpy(SDB[step_phase + 0].Fquat);
                    ...
                    if(del_pos.z > 0) del_pos.z = 0;
                    half_pelv_w = pelv_w/2 + 0.1*(fabs(del_pos.z)/20.0);
                    ...
                    if(del_pos.z < 0) stancF_to_nextF_local_limited.y = stancF_to_nextF_local_limited.y - 0.1*(fabs(del_pos.z)/20.0);

                    vec3 stancF_to_nextF_global = g_R_stanceF*stancF_to_nextF_local_limited;

                    vec3 nextF_global = pLF_ref + stancF_to_nextF_global;

                    SD_last.Fpos.x = (pLF_ref.x + nextF_global.x)/2; // middle Foot
                    SD_last.Fpos.y = (pLF_ref.y + nextF_global.y)/2; // middle Foot
                ...
                if(SDB[step_phase].swingFoot == LFoot){
                    ...
                    if(del_pos.z > 0) stancF_to_nextF_local_limited.y = stancF_to_nextF_local_limited.y + 0.1;

                    vec3 stancF_to_nextF_global = g_R_stanceF*stancF_to_nextF_local_limited;

                    vec3 nextF_global = pRF_ref + stancF_to_nextF_global;

                    SD_last.Fpos.x = (pRF_ref.x + nextF_global.x)/2; // middle Foot
                    SD_last.Fpos.y = (pRF_ref.y + nextF_global.y)/2; // middle Foot
                ...
                for(int i=0 ; i<2 ; i++){
                    ...
                    if(SDB[step_phase + i + 0].swingFoot == RFoot){
                        ...
                        if(del_pos.z > 0) del_pos.z = 0;
                        half_pelv_w = pelv_w/2 + 0.03*(fabs(del_pos.z)/20.0);
...
//        CP_error_lf = Alpha_CPelf*CP_error_lf + (1.-Alpha_CPelf)*f_R_g*((COM_m_filtered - COM_SA_ref) + (dCOM_m_diff_filtered/w - dCOM_SA_ref/w));
        ...
        vec3 CP_error_local_for_y = f_R_g*((COM_m_filtered - COM_SA_ref) + 0.7*(dCOM_m_diff_filtered/w - dCOM_SA_ref/w));
...
//        CP_error_lf.y = CP_error_lf.y*0.9;
        ...
        // CP error clampin
        double MAX_CP_error_y = 0.09;
        if(CP_error_lf.y > MAX_CP_error_y) CP_error_lf.y = MAX_CP_error_y;
        if(CP_error_lf.y < -MAX_CP_error_y) CP_error_lf.y = -MAX_CP_error_y;

//        if(CP_error_lf.x > 0.13) CP_error_lf.x = 0.13;
//        if(CP_error_lf.x < -0.13) CP_error_lf.x = -0.13;

        double MAX_CP_error_x = 0.2;
        if(CP_error_lf.x > MAX_CP_error_x) CP_error_lf.x = MAX_CP_error_x;
        if(CP_error_lf.x < -MAX_CP_error_x) CP_error_lf.x = -MAX_CP_error_x;
        ...
        cZMP_SA_lf = zmpProjectionTo_StanceFoot(RFoot, cZMP_SA_lf_before_saturate,-0.08, -0.04);
...
//        CP_error_rf.y = CP_error_rf.y*0.9;
        ...
        //CP error clamping
        double MAX_CP_error_y = 0.09;
        if(CP_error_rf.y > MAX_CP_error_y) CP_error_rf.y = MAX_CP_error_y;
        if(CP_error_rf.y < -MAX_CP_error_y) CP_error_rf.y = -MAX_CP_error_y;

        double MAX_CP_error_x = 0.2;
        if(CP_error_rf.x > MAX_CP_error_x) CP_error_rf.x = MAX_CP_error_x;
        if(CP_error_rf.x < -MAX_CP_error_x) CP_error_rf.x = -MAX_CP_error_x;
        ...
        // cZMP saturation into left foot
        cZMP_SA_rf = zmpProjectionTo_StanceFoot(LFoot, cZMP_SA_rf_before_saturate,-0.08, -0.04);
...
//    }
    ...
    //// -------------------------------------- Foot trajectory ----------------------------
    FootUp_height = 0.032/0.4*t_step;
    //for rigid foot
//    double Landing_Threshold = 40; // N
    //for damping foot
    double Landing_Threshold = 20; // N
    ...
    if(SDB[step_phase].swingFoot == RFoot){
        ...
        Landing_delXY = vec3();//Calc_Landing_delXY(RFoot, pLF_ref, SDB[step_phase+1].Fpos, qLF_ref, qRF_ref, G_R_g_pitroll);
...
//            if(RS.F_RF.z <= 70 && t_now < t_step*0.5 && RF_landing_flag == true){
            ...
            if(F_RF_filtered.z > Landing_Threshold && t_now > T_nom*0.6 && t_now < T_nom - t_half_dsp*2 && RF_landing_flag == false){
                ...
                RF_z_dz_ddz_old[1] = RF_z_dz_ddz_old[1]*0.0;
                ...
                RF_x_dx_ddx_old[1] = RF_x_dx_ddx_old[1]*0.0;

                // y
                pRF_landing.y = RF_y_dy_ddy_old[0];// + HB_sign(RF_y_dy_ddy_old[1])*0.002; // Landing y
                RF_y_dy_ddy_old[1] = RF_y_dy_ddy_old[1]*0.0;
            ...
            if(RF_landing_flag == false || t_now < T_nom*0.5){
                // Z direction
               RF_z_dz_ddz = FootZ_trajectory(t_step, t_now, dsp_ratio, RF_z_dz_ddz_old, FootUp_height + LandingZ_des, LandingZ_des+0.004);
               // X direction
            ...
            if(RF_landing_flag == true && t_now > T_nom*0.4){
                ...
                RF_z_dz_ddz = vec3(RF_z_dz_ddz_old[0]+RF_z_dz_ddz_old[1]*dt, RF_z_dz_ddz_old[1]*0.0, 0);
                // X direction
                //RF_x_dx_ddx = FootXY_Landing_trajectory(RF_landing_time, t_now, RF_x_dx_ddx_old, pRF_landing.x);
                RF_x_dx_ddx = vec3(RF_x_dx_ddx_old[0]+RF_x_dx_ddx_old[1]*dt, RF_x_dx_ddx_old[1]*0.0, 0);
                // Y direction
                //RF_y_dy_ddy = FootXY_Landing_trajectory(RF_landing_time, t_now, RF_y_dy_ddy_old, pRF_landing.y);
                RF_y_dy_ddy = vec3(RF_y_dy_ddy_old[0]+RF_y_dy_ddy_old[1]*dt, RF_y_dy_ddy_old[1]*0.0, 0);
    ...
    //            L_Vib_Control_init();
        ...
        Landing_delXY = vec3();//Calc_Landing_delXY(LFoot, SDB[step_phase+1].Fpos, pRF_ref, qLF_ref, qRF_ref, G_R_g_pitroll);
...
//            if(RS.F_LF.z <= 70 && t_now < t_step*0.5 && LF_landing_flag == true){
            ...
            if(F_LF_filtered.z > Landing_Threshold && t_now > T_nom*0.6 && t_now < T_nom - t_half_dsp*2 && LF_landing_flag == false){
                ...
                LF_z_dz_ddz_old[1] = LF_z_dz_ddz_old[1]*0.0;
                ...
                LF_x_dx_ddx_old[1] = LF_x_dx_ddx_old[1]*0.0;

                // y
                pLF_landing.y = LF_y_dy_ddy_old[0];// + HB_sign(LF_y_dy_ddy_old[1])*0.002; // Landing y
                LF_y_dy_ddy_old[1] = LF_y_dy_ddy_old[1]*0.0;
            ...
            if(LF_landing_flag == false || t_now < T_nom*0.5){
                // Z direction
                LF_z_dz_ddz = FootZ_trajectory(t_step, t_now, dsp_ratio, LF_z_dz_ddz_old, FootUp_height + LandingZ_des, LandingZ_des+0.00);
                // X direction
...
//                cout<<"Lf landing x: "<<pLF_landing.x<<"  pLF_landing.y: "<<pLF_landing.y<<endl;
                ...
                LF_z_dz_ddz = vec3(LF_z_dz_ddz_old[0]+LF_z_dz_ddz_old[1]*dt, LF_z_dz_ddz_old[1]*0.0, 0);
                // X direction
                //LF_x_dx_ddx = FootXY_Landing_trajectory(LF_landing_time, t_now, LF_x_dx_ddx_old, pLF_landing.x);
                LF_x_dx_ddx = vec3(LF_x_dx_ddx_old[0]+LF_x_dx_ddx_old[1]*dt, LF_x_dx_ddx_old[1]*0.0, 0);
                // Y direction
                //LF_y_dy_ddy = FootXY_Landing_trajectory(LF_landing_time, t_now, LF_y_dy_ddy_old, pLF_landing.y);
                LF_y_dy_ddy = vec3(LF_y_dy_ddy_old[0]+LF_y_dy_ddy_old[1]*dt, LF_y_dy_ddy_old[1]*0.0, 0);
            }
    ...
    //            R_Vib_Control_init();
        ...
        if(step_phase > N_step -1){
            RF_z_dz_ddz[1] = -RF_z_dz_ddz[0]/0.3;

            RF_z_dz_ddz[0] = RF_z_dz_ddz[0] + RF_z_dz_ddz[1]*dt;

            LF_z_dz_ddz[1] = -LF_z_dz_ddz[0]/0.3;

            LF_z_dz_ddz[0] = LF_z_dz_ddz[0] + LF_z_dz_ddz[1]*dt;
        }
    ...
    if(WST_rad > 30.0*D2R) WST_rad = 30.0*D2R;
    if(WST_rad < -30.0*D2R) WST_rad = -30.0*D2R;

    WST_ref_deg = WST_rad*R2D/2;


   RSP_des_deg = -WST_ref_deg*3/2;
   LSP_des_deg = WST_ref_deg*3/2;
    ...
    DSP_Fz_controller3(SDB[step_phase].swingFoot, RS.F_RF.z, RS.F_LF.z, RS.IMUangle); // this functino gives z_ctrl;
//    DSP_Fz_controller2(SDB[step_phase].swingFoot);


    if(DSP_FZ_Control_flag == true){
        pRF_ref.z = RF_z_dz_ddz[0] + 0.5*z_ctrl_filtered + z_com_ctrl;
        pLF_ref.z = LF_z_dz_ddz[0] - 0.5*z_ctrl_filtered + z_com_ctrl;
    }
    else{
...
//    RF_landing_angle_gain_pitch = 10*40/(40 + RS.F_RF.z);
    ...
    if(RF_landing_flag == false){
        dRF_landing_angle.r = 0.01*(-G_R_g_pitroll_rpy.r) - RF_landing_angle.r/0.1;
        dRF_landing_angle.p = 0.01*(-G_R_g_pitroll_rpy.p) - RF_landing_angle.p/0.1;
    }
    else{
    ...
    if(RF_landing_angle.r >= 5*D2R) RF_landing_angle.r = 5*D2R;
    if(RF_landing_angle.r <= -5*D2R) RF_landing_angle.r = -5*D2R;
    if(RF_landing_angle.p >= 5*D2R) RF_landing_angle.p = 5*D2R;
    if(RF_landing_angle.p <= -5*D2R) RF_landing_angle.p = -5*D2R;
...
//    LF_landing_angle_gain_pitch = 12*50/(50 + RS.F_LF.z);
    ...
    if(LF_landing_flag == false){
        dLF_landing_angle.r = 0.01*(-G_R_g_pitroll_rpy.r) - LF_landing_angle.r/0.1;
        dLF_landing_angle.p = 0.01*(-G_R_g_pitroll_rpy.p) - LF_landing_angle.p/0.1;
    }
    else{
    ...
    if(LF_landing_angle.r >= 5*D2R) LF_landing_angle.r = 5*D2R;
    if(LF_landing_angle.r <= -5*D2R) LF_landing_angle.r = -5*D2R;
    if(LF_landing_angle.p >= 5*D2R) LF_landing_angle.p = 5*D2R;
    if(LF_landing_angle.p <= -5*D2R) LF_landing_angle.p = -5*D2R;
...
//    }
    ...
    if(Joint_deflection_compensation_flag == true){
        if(SDB[step_phase].swingFoot == RFoot){
            ...
            // hip roll
            double alpha = 1/(1 + 2*PI*dt*5);

            //L_roll_compen_deg = alpha*L_roll_compen_deg + (1-alpha)*HB_sign(ZMP_local.y)*0.1*8;
            L_roll_compen_deg = alpha*L_roll_compen_deg + (1-alpha)*COM_local.y*10;
        ...
        if(SDB[step_phase].swingFoot == LFoot){
            ...
            // hip roll
            double alpha = 1/(1 + 2*PI*dt*5);
            //R_roll_compen_deg = alpha*R_roll_compen_deg + (1-alpha)*HB_sign(ZMP_local.y)*0.1*8;
            R_roll_compen_deg = alpha*R_roll_compen_deg + (1-alpha)*COM_local.y*10;
...
void HB_PreviewWalk::MeasurementInput(RState _RS){
    RS = _RS;
...
vec3 HB_PreviewWalk::COM_measurment(vec3 _COM, double _zc, vec3 _ZMP_global, vec3 _IMUangle, quat _qPel){
