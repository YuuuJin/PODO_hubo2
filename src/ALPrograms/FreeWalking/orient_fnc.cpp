...
extern pRBCORE_SHM_SENSOR      sharedData;*/
...
int convert_euler_FT(double _pre_state,double pRF_3x1[], double pLF_3x1[],double qRF_4x1[], double qLF_4x1[], double control_result_RF[],double control_result_LF[])
{
    ...
    M_LF[0] =  sharedData->FT[LAFT].Mx -0.03*sharedData->FT[LAFT].Fy;
    M_LF[1] =  sharedData->FT[LAFT].My -0.03*sharedData->FT[LAFT].Fx;
    M_LF[2] =  sharedData->FT[LAFT].Mz;

    QTtransform(qLF_4x1,M_LF,M_LF_Global);

    M_RF[0] =  sharedData->FT[RAFT].Mx -0.03*sharedData->FT[RAFT].Fy;
    M_RF[1] =  sharedData->FT[RAFT].My -0.03*sharedData->FT[RAFT].Fx;
