...
double Gyro_Ankle_FeedBack_ONOFF = 0.0;
...
bool TorsoAngle_Comp_ONOFF = false;
...
double sharedData_FOG_Roll = 0;
double sharedData_FOG_Pitch = 0;

double InitialComz =0;

const double L_FOOT = 0.28;//0.24;
double L_PEL2PEL = 0.0885*2;//0.26;
...
inline double currentRef(int jnum){
    return sharedData->ENCODER[joint->Joints[jnum]->GetId()][joint->Joints[jnum]->GetCh()].CurrentReference;
}

inline double currentPositIK_LowerBodyion(int jnum){
    return sharedData->ENCODER[joint->Joints[jnum]->GetId()][joint->Joints[jnum]->GetCh()].CurrentPosition;
}
...
int pyeongchang_foot_print();
int pyeongchang_foot_print_0st(int _num,int _foot);
int pyeongchang_foot_print_1st(int _num,int _foot);
int pyeongchang_foot_print_2nd(int _num,int _foot);

int pyeongchang_foot_print_1st_cwrot(int _num,int _foot);
int pyeongchang_foot_print_2nd_cwrot(int _num,int _foot);
...
//    fsm->DEL_T = 0.005;
    ...
    des_pCOM_3x1[0] = userData->WalkReadyCOM[0] = 0.02;//0.0;//0.0237f;
    des_pCOM_3x1[1] = userData->WalkReadyCOM[1] = 0.0;
    des_pCOM_3x1[2] = userData->WalkReadyCOM[2] = 0.7;//0.55;//0.77;// + 0.11;//(61.8kg:0.74)//59//0.8;//71kg
    ...
    while(isTerminated == 0){
        ...
        usleep(100*1000);
        sharedData->COMMAND[PODO_NO].USER_PARA_INT[4] = INSIDE_WALKING;
        switch(sharedData->COMMAND[PODO_NO].USER_COMMAND)
        {

        case FREEWALK_PYEONGCHANG:
            if(fsmFlag == false)
            {
                Upperbody_Gain_Override();
                DSPHoldTime = 0.;
                L_PEL2PEL = 0.0885*2;//0.22;
                L_PEL2PEL_OFFSET =0.001;
                U0_Gain = 0.88;//1.06;
                cout << ">>> COMMAND: FREEWALK_WALK_PYEONGCHANG" << endl;
                ...
                fileName.sprintf("%s", sharedData->COMMAND[PODO_NO].USER_PARA_CHAR);
                GetGain(userData->WalkReadyCOM[2] + COM_Offset);
                fsm->walking_mode = NORMAL_WALKING;
                fsm->DSPScheduler.clear();

                double StepTime = 0.45;//45;
                fsm->TIME_DSP = StepTime*2./10.;
                fsm->TIME_SSP = StepTime*8./10.;
                getCurrentPos();
                pyeongchang_foot_print();
            ...
            sharedData->COMMAND[PODO_NO].USER_COMMAND = FREEWALK_NO_ACT;
        ...
        case FREEWALK_ADDMASS:
            Mass_ONOFF_Flag = sharedData->COMMAND[PODO_NO].USER_PARA_INT[1];
            sharedData->COMMAND[PODO_NO].USER_COMMAND = FREEWALK_NO_ACT;
            break;
        case FREEWALK_WALK:
        if(fsmFlag == false)
...
//                printf("Mass OFF !!!!!!!!!!!!!!!!!!!!!!!!!!!!m_RightWrist = %f,m_LeftWrist = %f \n",kine_drc_hubo4.m_RightWrist,kine_drc_hubo4.m_RightWrist);
            ...
            GetGain(userData->WalkReadyCOM[2] + COM_Offset);
            fsm->walking_mode = sharedData->COMMAND[PODO_NO].USER_PARA_INT[9];
            fsm->DSPScheduler.clear();
            if(fsm->walking_mode == NORMAL_WALKING){
                double StepTime = userData->G2M.StepTime;
                if(sharedData->COMMAND[PODO_NO].USER_PARA_INT[4] == INSIDE_WALKING)
                {
                    fsm->TIME_DSP = StepTime*2./10.;//1.5f;
                    fsm->TIME_SSP = StepTime*8./10.;//3.0f;
                }
                else if(sharedData->COMMAND[PODO_NO].USER_PARA_INT[4] == OUTSIDE_WALKING)
                {
...
//                if(fabs(userData->G2M.StepNum) > 30)userData->G2M.StepNum = 30;
                ...
                if(fabs(userData->G2M.StepLength) > 0.45)userData->G2M.StepLength = 0.45;
...
//                    printf("BLOCK CLIMB TASK speed set DSP: %f  SSP: %f \n",userData->terrain_variable[0],userData->terrain_variable[1]);
                    for(int i=0; i<sharedData->COMMAND[PODO_NO].USER_PARA_INT[5]; i++){
                        DSPTask task;
                        ...
                        task.COMz[1]    = userData->G2M.walkingDSP[i*15+13];//sharedData->COMMAND[PODO_NO].USER_PARA_DOUBLE[i*14+13];
...
//                sharedData->COMMAND[RBCORE_PODO_NO].USER_COMMAND = DAEMON_SENSOR_FOG_NULL;
                ...
                GetGain(userData->WalkReadyCOM[2] + COM_Offset);
                fsm->walking_mode = sharedData->COMMAND[PODO_NO].USER_PARA_INT[9];
                fsm->heel_pitching_mode = sharedData->COMMAND[PODO_NO].USER_PARA_INT[8];
                fsm->DSPScheduler.clear();
...
//                    printf("BLOCK CLIMB TASK speed set DSP: %f  SSP: %f \n",fsm->TIME_DSP,fsm->TIME_SSP);
                    ...
                    if(sharedData->COMMAND[PODO_NO].USER_PARA_INT[3] == FIELD_TERRAIN)
                    {
                        for(int i=0; i<sharedData->COMMAND[PODO_NO].USER_PARA_INT[5]; i++){
                            DSPTask task;
                            task.Left[0]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[i*gab];
                            task.Left[1]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[i*gab+1];
                            task.Left[2]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[i*gab+2];
                            task.LYaw       = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[i*gab+3];
                            task.LRoll      = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[i*gab+4];
                            task.LPitch     = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[i*gab+5];

                            task.Right[0]   = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[i*gab+6];
                            task.Right[1]   = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[i*gab+7];
                            task.Right[2]   = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[i*gab+8];
                            task.RYaw       = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[i*gab+9];
                            task.RRoll      = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[i*gab+10];
                            task.RPitch     = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[i*gab+11];

                            task.COMz[0]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[i*gab+12];
                            task.COMz[1]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[i*gab+13];
                            task.COMz[2]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[i*gab+14];
    ...
    //                        task.HTime      =  1.0f;
                    ...
                    else if(sharedData->COMMAND[PODO_NO].USER_PARA_INT[3] == GUI_TERRAIN)
                    {
                        for(int i=0; i<sharedData->COMMAND[PODO_NO].USER_PARA_INT[5]; i++){
                            DSPTask task;
                            ...
                            task.COMz[1]    = userData->G2M.walkingDSP[i*gab+13];//sharedData->COMMAND[PODO_NO].USER_PARA_DOUBLE[i*14+13];
...
//                    printf("BLOCK CLIMB TASK speed set DSP: %f  SSP: %f \n",fsm->TIME_DSP,fsm->TIME_SSP);
                    ...
                    for(int i=0; i<sharedData->COMMAND[PODO_NO].USER_PARA_INT[5]; i++){
                        ...
                        {
                            if(sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[150]> 50)
                            {
//                                printf("STAIR Left Foot FIRST");
                                task.Left[0]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab] + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[151];
                                task.Left[1]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+1] + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[152];
                                task.Left[2]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+2] + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157];
                                task.LYaw       = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+3];
                                task.LRoll      = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+4];
                                task.LPitch     = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+5];

                                task.Right[0]   = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+6];
                                task.Right[1]   = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+7];
                                task.Right[2]   = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+8];
                                task.RYaw       = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+9];
                                task.RRoll      = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+10];
                                task.RPitch     = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+11];

                                task.COMz[0]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13]+0.03f;
                                task.COMz[1]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13]+0.03f;
                                task.COMz[2]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13]+0.03f;

                                task.HTime      =  1.f;
                            }else
                            {
//                                printf("STAIR Right Foot Left");
                                task.Left[0]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab];
                                task.Left[1]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+1];
                                task.Left[2]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+2];
                                task.LYaw       = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+3];
                                task.LRoll      = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+4];
                                task.LPitch     = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+5];

                                task.Right[0]   = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+6]+ sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[151];
                                task.Right[1]   = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+7]+ sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[152];
                                task.Right[2]   = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+8]+ sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157];
                                task.RYaw       = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+9];
                                task.RRoll      = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+10];
                                task.RPitch     = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+11];
                        ...
                        }else if(i == 4)
                        {
                            ...
                            {
//                                printf("STAIR Left Foot FIRST");
                                task.Left[0]    = tempLP[0] = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab]  + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[151];
                                task.Left[1]    = tempLP[1] = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+1]  + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[152];
                                task.Left[2]    = tempLP[2] = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+2]  + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157];
                                task.LYaw       = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+3];
                                task.LRoll      = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+4];
                                task.LPitch     = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+5];

                                task.Right[0]   = tempRP[0] = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+6]  + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[151];
                                task.Right[1]   = tempRP[1] = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+7]  + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[152];
                                task.Right[2]   = tempRP[2] = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+8]  + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157];
                                task.RYaw       = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+9];
                                task.RRoll      = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+10];
                                task.RPitch     = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+11];


                                task.COMz[0]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13] + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157];
                                task.COMz[1]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13] + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157];
                                task.COMz[2]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13] + 0.03f;
...
//                                printf("STAIR Left Foot Left");
                        ...
                        }else if(i == 5)
                        {
                            ...
                            {
//                                printf("STAIR Left Foot FIRST");
                                task.Left[0]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab]  + 2.0f*sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[151];
                                task.Left[1]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+1]  + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[152];
                                task.Left[2]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+2]  + 2.0f*sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157];
                                task.LYaw       = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+3];
                                task.LRoll      = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+4];
                                task.LPitch     = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+5];
                                ...
                                task.COMz[0]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13] + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157] + 0.03f;
                                task.COMz[1]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13] + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157] + 0.03f;
                                task.COMz[2]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13] + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157] + 0.03f;
...
//                                printf("STAIR Right Foot Left");
                                ...
                                task.Right[0]   = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+6] + 2.0f*sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[151];
                                task.Right[1]   = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+7] + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[152];
                                task.Right[2]   = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+8] + 2.0f*sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157];
                                task.RYaw       = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+9];
                                task.RRoll      = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+10];
                                task.RPitch     = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+11];
...
//                                printf("STAIR Left Foot FIRST");
                                ...
                                task.COMz[0]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13] + 2.0f*sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157];
                                task.COMz[1]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13] + 2.0f*sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157];
                                task.COMz[2]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13] + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157] + 0.03f;
...
//                                printf("STAIR Right Foot Left");
                        ...
                        }else if(i == 7)
                        {
                            ...
                            {
//                                printf("STAIR Left Foot FIRST");
                                task.Left[0]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab]  + 2.0f*sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[151] + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[151];
                                task.Left[1]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+1]  + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[152] ;
                                task.Left[2]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+2]  + 2.0f*sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157] + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[158];
                                task.LYaw       = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+3];
                                task.LRoll      = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+4];
                                task.LPitch     = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+5];
                                ...
                                task.COMz[0]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13] + 2.0f*sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157] + 0.03f;
                                task.COMz[1]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13] + 2.0f*sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157] + 0.03f;
                                task.COMz[2]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13] + 2.0f*sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157] + 0.03f;
...
//                                printf("STAIR Right Foot Left");
                                ...
                                task.Right[0]   = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+6] + 2.0f*sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[151] +sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[151];
                                task.Right[1]   = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+7] + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[152];
                                task.Right[2]   = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+8] + 2.0f*sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157] + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[158];
                                task.RYaw       = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+9];
                                task.RRoll      = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+10];
                                task.RPitch     = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+11];
...
//                                printf("STAIR Left Foot FIRST");
                                ...
                                task.COMz[0]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13] + 2.0f*sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157] + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[158];
                                task.COMz[1]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13] + 2.0f*sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157] + sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[158];
                                task.COMz[2]    = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2*gab+13] + 2.0f*sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[157] + 0.03f;
...
//        sharedData_FOG_Roll = sharedData->FOG.Roll*R2D;
//        sharedData_FOG_Pitch = sharedData_FOG_Pitch*R2D;


        if(fsmFlag == true)
        {
            if(fabs(sharedData_FOG_Roll)>10||fabs(sharedData_FOG_Pitch)>10)
            {
//                Fall_cnt++;
//                if(Fall_cnt>10)
//                {
//                    Fall_Flag = true;
//                    if(sharedData_FOG_Roll>0)  Fall_Roll_DIR = FALL_PLUS;
//                    else Fall_Roll_DIR = FALL_MINUS;

//                    if(sharedData->FOG.Pitch>0)  Fall_Pitch_DIR = FALL_PLUS;
//                    else Fall_Roll_DIR = FALL_MINUS;
//                }
            ...
            if(Fall_Flag == false)
            {
                ...
                LF_FZ_LPF =(1.f-2.f*PI*7.0*(double)RT_TIMER_PERIOD_MS*0.001f)*LF_FZ_LPF + 2.f*PI*7.0*(double)RT_TIMER_PERIOD_MS*0.001f*sharedData->FT[LAFT].Fz;
                RF_FZ_LPF =(1.f-2.f*PI*7.0*(double)RT_TIMER_PERIOD_MS*0.001f)*RF_FZ_LPF + 2.f*PI*7.0*(double)RT_TIMER_PERIOD_MS*0.001f*sharedData->FT[RAFT].Fz;

                LF_MX_LPF =(1.f-2.f*PI*7.0*(double)RT_TIMER_PERIOD_MS*0.001f)*LF_MX_LPF + 2.f*PI*7.0*(double)RT_TIMER_PERIOD_MS*0.001f*sharedData->FT[LAFT].Mx;
                RF_MX_LPF =(1.f-2.f*PI*7.0*(double)RT_TIMER_PERIOD_MS*0.001f)*RF_MX_LPF + 2.f*PI*7.0*(double)RT_TIMER_PERIOD_MS*0.001f*sharedData->FT[RAFT].Mx;

                LF_MY_LPF =(1.f-2.f*PI*7.0*(double)RT_TIMER_PERIOD_MS*0.001f)*LF_MY_LPF + 2.f*PI*7.0*(double)RT_TIMER_PERIOD_MS*0.001f*sharedData->FT[LAFT].My;
                RF_MY_LPF =(1.f-2.f*PI*7.0*(double)RT_TIMER_PERIOD_MS*0.001f)*RF_MY_LPF + 2.f*PI*7.0*(double)RT_TIMER_PERIOD_MS*0.001f*sharedData->FT[RAFT].My;
                get_zmp();
                get_zmp2();
                JW_save();
                JW_save2();

//                printf("------ %f ------",sharedData->FT[RAFT].Pitch);
...
//                GotoFallPos();
            ...
            if(fabs(sharedData_FOG_Roll)>10||fabs(sharedData_FOG_Pitch)>10||stopFlag==true)
            {
                Upperbody_Gain_Lock();
                fsmFlag = false;
                Fall_cnt =0;
...
void RBFlagThread(void *)
{
    rt_task_set_periodic(NULL, TM_NOW, 900*1000);        // 300 usec
...
//    printf("Qub = %f,%f,%f,%f,%f,%f,%f,%f\n",Qub[idRSR],Qub[idRSP],Qub[idRSY],Qub[idREB],Qub[idRWY],Qub[idRWP],Qub[idRWY],Qub[idRWY2]);
...
//    static double IMU_NULL_FLAG = 0;
    static double EarlyLanding_FT = 80.;
...
//    static int add_mass_FLAG = 0;
    ...
    TorsoOri[0] = sharedData_FOG_Roll*D2R;
    TorsoOri[1] = sharedData_FOG_Pitch*D2R;
    TorsoOri[2] = 0.;
...
//            GainGyro = 0.5*(1-cos(PI*(fsm_state_timer[fsm->StateInfos[0][0]])/fsm->TIME_DSP));
        ...
        Kirk_Control();
        AnkleRollAngle[HUBO_RIGHT]  = -HUBO2TorqInitConRAR(0, sharedData->FT[RAFT].Mx - sharedData->FT[RAFT].Fy*0.03, 1,0.0003);
        AnkleRollAngle[HUBO_LEFT]   = -HUBO2TorqInitConLAR(0, sharedData->FT[LAFT].Mx - sharedData->FT[LAFT].Fy*0.03, 1,0.0003);
        AnklePitchAngle[HUBO_RIGHT] = HUBO2TorqInitConRAP(0.0, -1.0f*(sharedData->FT[RAFT].My -sharedData->FT[LAFT].My - sharedData->FT[RAFT].Fx*0.03 - sharedData->FT[LAFT].Fx*0.03     ), 1,0.0015);
...
//            convert_euler(des_pRF_3x1_n, des_pLF_3x1_n, sharedData_FOG_Roll*D2R, sharedData_FOG_Pitch*D2R, 0,BTW_FOOT_Angle_roll, BTW_FOOT_Angle_pitch, BTW_FOOT_Angle_yaw,BTW_FOOT_qPEL_comp_4x1);
//            convert_euler(des_pRF_3x1_n, des_pLF_3x1_n, sharedData_FOG_Roll*D2R*cos(yaw_angle*D2R) - sharedData_FOG_Pitch*D2R*sin(yaw_angle*D2R), sharedData_FOG_Roll*D2R*sin(yaw_angle*D2R) + sharedData_FOG_Pitch*D2R*cos(yaw_angle*D2R), 0,BTW_FOOT_Angle_roll, BTW_FOOT_Angle_pitch, BTW_FOOT_Angle_yaw,BTW_FOOT_qPEL_comp_4x1);
        convert_euler(des_pRF_3x1_n, des_pLF_3x1_n, sharedData_FOG_Roll*D2R*cos(-yaw_angle*D2R)-sharedData_FOG_Pitch*D2R*sin(-yaw_angle*D2R), sharedData_FOG_Roll*D2R*sin(-yaw_angle*D2R)+sharedData_FOG_Pitch*D2R*cos(-yaw_angle*D2R), 0,BTW_FOOT_Angle_roll, BTW_FOOT_Angle_pitch, BTW_FOOT_Angle_yaw,BTW_FOOT_qPEL_comp_4x1);
...
//        TorsoPitchAngle = HUBO2TorsoInitConPitch(0.0, AnglePitch, 1);
        ...
        */
        /*
        HUBO2_ForceInitCon[RIGHT][Xdir] = -HUBO2_ForceInitCon_RF_X(0 , (sharedData->FT[RAFT].Fx-sharedData->FT[LAFT].Fx), 1);
        HUBO2_ForceInitCon[LEFT][Xdir]=-HUBO2_ForceInitCon[RIGHT][Xdir];
        HUBO2_ForceInitCon[RIGHT][Ydir] = -HUBO2_ForceInitCon_RF_Y(0 , sharedData->FT[RAFT].Fy, 1);
        //HUBO2_ForceInitCon[LEFT][Xdir]  = -HUBO2_ForceInitCon_LF_X(0 , sharedData->FT[LAFT].Fx, 1);
        HUBO2_ForceInitCon[LEFT][Ydir]  = -HUBO2_ForceInitCon_LF_Y(0 , sharedData->FT[LAFT].Fy, 1);

        //if(fabs(sharedData->FT[RAFT].Mx - sharedData->FT[RAFT].Fy*0.03)>30)
//            AnkleRollAngle[HUBO_RIGHT]  = -HUBO2TorqInitConRAR(0, sharedData->FT[RAFT].Mx - sharedData->FT[RAFT].Fy*0.03, 1,0.0003);
//        else
        AnkleRollAngle[HUBO_RIGHT]  = -HUBO2TorqInitConRAR(0, sharedData->FT[RAFT].Mx - sharedData->FT[RAFT].Fy*0.03, 1,0.0002);
        AnkleRollAngle[HUBO_LEFT]   = -HUBO2TorqInitConLAR(0, sharedData->FT[LAFT].Mx - sharedData->FT[LAFT].Fy*0.03, 1,0.0002);
        AnklePitchAngle[HUBO_RIGHT] = HUBO2TorqInitConRAP(0.0, -1.0f*(sharedData->FT[RAFT].My -sharedData->FT[LAFT].My - sharedData->FT[RAFT].Fx*0.03 - sharedData->FT[LAFT].Fx*0.03     ), 1,0.0011);
        */
...
//            FullGainRightLeg();
        ...
        {
            if(sharedData_FOG_Roll <-2.0)
            {
                U0_Gain_Goal_KI += 0.02*(sharedData_FOG_Roll);
                printf("ugain_KI = %f\n",U0_Gain_KI);
            }
...
//        yaw_angle_last = yaw_angle;
        ...
        {
//            printf("!!!!!!!!yaw\n");
            if(sharedData->COMMAND[PODO_NO].USER_PARA_INT[4] == OUTSIDE_WALKING)
            {
...
//            QTtransform(des_qRF_4x1,M_RF,M_RF_Global);
            if(pre_state == HUBO_RIGHT)
            {
                ...
                {
                    WBIK_Torq[RIGHT][Xdir] = -WBIKTorqInitConRAR(0, M_RF[0] - sharedData->FT[RAFT].Fy*0.00, 1,0.0003);
                    WBIK_Torq[LEFT][Xdir]  = 0.;
                    WBIK_Torq[RIGHT][Ydir] = -WBIKTorqInitConRAP(0, M_RF[1] - sharedData->FT[RAFT].Fx*0.00, 1,0.0003);
            ...
            else if(pre_state == HUBO_LEFT)
            {
                ...
                {
                    WBIK_Torq[LEFT][Xdir]   = -WBIKTorqInitConLAR(0, M_LF[0] - sharedData->FT[LAFT].Fy*0.00, 1,0.0003);
                    WBIK_Torq[RIGHT][Xdir]  = 0.;
                    WBIK_Torq[LEFT][Ydir]   = -WBIKTorqInitConLAP(0, M_LF[1] - sharedData->FT[LAFT].Fx*0.00, 1,0.0003);
...
//        printf("WBIK_Torq Roll = %f,%f, Pitch  = %f,%f\n",WBIK_Torq[RIGHT][Xdir],WBIK_Torq[LEFT][Xdir],WBIK_Torq[RIGHT][Ydir],WBIK_Torq[LEFT][Ydir]);
            ...
            if(fsm_state_timer[fsm->StateInfos[0][0]]>fsm->TIME_DSP*.0/5.)
            {
                ...
                AnkleRollAngle[HUBO_LEFT]   = -HUBO2TorqInitConLAR(0, sharedData->FT[LAFT].Mx - sharedData->FT[LAFT].Fy*0.03, 1,0.0003);
                    ...
                    AnklePitchAngle[HUBO_RIGHT] = HUBO2TorqInitConRAP(0.0, -1.0f*(sharedData->FT[RAFT].My -sharedData->FT[LAFT].My - sharedData->FT[RAFT].Fx*0.03 - sharedData->FT[LAFT].Fx*0.03     ), 1,0.0015);*/
//                    if(fsm->walking_mode == TERRAIN_WALKING_ONE_STEP)
//                    AnklePitchAngle[HUBO_LEFT] = HUBO2TorqInitConLAP(0.0, 1.0f*(sharedData->FT[RAFT].My -sharedData->FT[LAFT].My - sharedData->FT[RAFT].Fx*0.03 - sharedData->FT[LAFT].Fx*0.03     ), 1,0.0015);
...
//        AnkleRollAngle[HUBO_RIGHT]  = -HUBO2TorqInitConRAR(0, sharedData->FT[RAFT].Mx - sharedData->FT[RAFT].Fy*0.03, 1,0.0002);
        ...
        if(pre_state == HUBO_RIGHT)
        {
            ...
            if(fsm->walking_mode == TERRAIN_WALKING_ONE_STEP)
            {
                ...
                WBIK_Torq[RIGHT][Ydir] = result_RF[1];
                WBIK_Torq[LEFT][Ydir]  = result_LF[1];
//                WBIK_Torq[RIGHT][Xdir] = -WBIKTorqInitConRAR(0, M_RF[0] - sharedData->FT[RAFT].Fy*0.003, 1,0.0003);
//                WBIK_Torq[LEFT][Xdir]  = -WBIKTorqInitConLAR(0, M_LF[0] - sharedData->FT[LAFT].Fy*0.003, 1,0.0003);
//                WBIK_Torq[RIGHT][Ydir] = -WBIKTorqInitConRAP(0, M_RF[1] - M_LF[1] - sharedData->FT[RAFT].Fx*0.003 - sharedData->FT[LAFT].Fx*0.003, 1,0.0003);
//                WBIK_Torq[LEFT][Ydir]  = 0.;
            }
            else
            {
...
//                WBIK_Torq[LEFT][Xdir]   = -WBIKTorqInitConLAR(0, M_LF[0] - sharedData->FT[LAFT].Fy*0.003, 1,0.0003);
//                WBIK_Torq[RIGHT][Xdir]  = -WBIKTorqInitConRAR(0, M_RF[0] - sharedData->FT[RAFT].Fy*0.003, 1,0.0003);
//                WBIK_Torq[LEFT][Ydir]   = WBIKTorqInitConLAP(0, M_RF[1] - M_LF[1] - sharedData->FT[RAFT].Fx*0.003 - sharedData->FT[LAFT].Fx*0.003, 1,0.0003);
//                WBIK_Torq[RIGHT][Ydir]  = 0.;
            }
            else
            {
...
//    printf("WBIK_Torq Roll = %f,%f, Pitch  = %f,%f\n",WBIK_Torq[RIGHT][Xdir],WBIK_Torq[LEFT][Xdir],WBIK_Torq[RIGHT][Ydir],WBIK_Torq[LEFT][Ydir]);
//        WBIK_Torq[RIGHT][Ydir] = WBIKTorqInitConRAP(0.0, -1.0f*(sharedData->FT[RAFT].My -sharedData->FT[LAFT].My - sharedData->FT[RAFT].Fx*0.03 - sharedData->FT[LAFT].Fx*0.03     ), 1,0.0015);
...
//        I_ZMP_CON_Y = Y_ZMP_integral_in_static(1);
        ...
        //if(fsm_state_timer[fsm->StateInfos[0][0]]<10)
            ...
            {
                if(fabs(sharedData_FOG_Pitch)<0.1 && fabs(sharedData_FOG_Roll)<0.1)
                {
                    fsmFlag =false;
...
//                AnkleRollAngle[HUBO_RIGHT]  = -HUBO2TorqInitConRAR(0, sharedData->FT[RAFT].Mx + sharedData->FT[RAFT].Mx*sin(fsm->RightInfos[0][4]*D2R)*0.03, 1);
//                AnkleRollAngle[HUBO_LEFT]   = -HUBO2TorqInitConLAR(0, sharedData->FT[LAFT].Mx + sharedData->FT[LAFT].Mx*sin(fsm->LeftInfos[0][4]*D2R)*0.03, 1);
//                AnklePitchAngle[HUBO_RIGHT] = HUBO2TorqInitConRAP(0.0, -1.0f*(sharedData->FT[RAFT].My -sharedData->FT[LAFT].My + sharedData->FT[RAFT].Mx*sin(fsm->RightInfos[0][5]*D2R)*0.03 + sharedData->FT[LAFT].Mx*sin(fsm->LeftInfos[0][5]*D2R)*0.03), 1);
//                HUBO2_ForceInitCon[RIGHT][Xdir] = -HUBO2_ForceInitCon_RF_X(0 , sharedData->FT[RAFT].Fx, 1);
                HUBO2_ForceInitCon[RIGHT][Xdir] = -HUBO2_ForceInitCon_RF_X(0 , (sharedData->FT[RAFT].Fx-sharedData->FT[LAFT].Fx), 1);
                HUBO2_ForceInitCon[LEFT][Xdir]  = -HUBO2_ForceInitCon[RIGHT][Xdir];
                HUBO2_ForceInitCon[RIGHT][Ydir] = -HUBO2_ForceInitCon_RF_Y(0 , sharedData->FT[RAFT].Fy, 1);
//                HUBO2_ForceInitCon[LEFT][Xdir]  = -HUBO2_ForceInitCon_LF_X(0 , sharedData->FT[LAFT].Fx, 1);
...
//            else if(pre_state == HUBO_LEFT)
//            {
//                AnkleRollAngle[HUBO_LEFT]   = -HUBO2TorqInitConLAR(0, sharedData->FT[LAFT].Mx, 1);
//                AnkleRollAngle[HUBO_RIGHT]  = -HUBO2TorqInitConRAR(0, sharedData->FT[RAFT].Mx, 1);
//                AnklePitchAngle[HUBO_RIGHT] = HUBO2TorqInitConLAP(0.0, -1.0f*(sharedData->FT[RAFT].My-sharedData->FT[LAFT].My), 1);
//            }
...
//            TorsoPitchAngle = HUBO2TorsoInitConPitch(0.0, AnglePitch, 1);
    ...
    convert_euler_imu(RF_PEL, LF_PEL, sharedData_FOG_Roll*D2R, sharedData_FOG_Pitch*D2R, 0,BTW_PEL_angle_roll, BTW_PEL_angle_pitch, BTW_PEL_angle_yaw);
    convert_euler_imu(RF_PEL, LF_PEL, sharedData->IMU[0].RollVel*D2R, sharedData->IMU[0].PitchVel*D2R, 0,BTW_PEL_angle_roll_vel, BTW_PEL_angle_pitch_vel, BTW_PEL_angle_yaw_vel);

    FOGRollVel_NF2 = NotchFilter_GyroRollVel(sharedData->IMU[0].RollVel,1);
    FOGPitchVel_NF2 = NotchFilter_GyroPitchVel(sharedData->IMU[0].PitchVel,1);
...
//    pgain = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[0];
//    dgain = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[1];

//    pgain_2 = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[2];
//    dgain_2 = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[3];
        if(sharedData->COMMAND[PODO_NO].USER_PARA_INT[4] == OUTSIDE_WALKING)
        {
            //only 1st order butterworth filter pitch pgain = 2.1, dgain = 10.
            //only 1st order butterworth filter roll  pgain = 2. , dgain = 7.
            AnkleControl1 = (sharedData_FOG_Roll*2.  + sharedData->IMU[0].RollVel*6.);
            AnkleControl2 = (sharedData_FOG_Pitch*2. + sharedData->IMU[0].PitchVel*4.);
//       AnkleControl1 = (sharedData_FOG_Roll*2.1 + FOGRollVel_LPF*19.4);
//       AnkleControl2 = (sharedData_FOG_Pitch*1.8 + FOGPitchVel_LPF*5.1);
        }
        else
        {
            if(sharedData->COMMAND[PODO_NO].USER_PARA_INT[4] == LADDER_WALKING)
            {
                AnkleControl1 = (sharedData_FOG_Roll*2.3 + FOGRollVel_LPF*6.);
                AnkleControl2 = (sharedData_FOG_Pitch*2.3 + FOGPitchVel_LPF*4.);
            }else if(sharedData->COMMAND[PODO_NO].USER_PARA_INT[4] == GOAL_WALKING)
            {
                AnkleControl1 = (sharedData_FOG_Roll*3.7f + sharedData->IMU[0].RollVel*6.);
                AnkleControl2 = (sharedData_FOG_Pitch*3.7f + sharedData->IMU[0].PitchVel*4.);
            }else
            {
                AnkleControl1 = (sharedData->IMU[0].Roll*0.3f + FOGRollVel_LPF*0.05);
                AnkleControl2 = (sharedData->IMU[0].Pitch*0.3f + FOGPitchVel_LPF*0.05);
            }
...
//        CONT_X = JW_InvPattern_U[1]*U_Gain + (GLOBAL_X_LIPM_n)*(1-U_Gain)      +(- 0.001*Del_PC_X_DSP_XZMP_CON*G_DSP_X*(1 - fabs(GLOBAL_X_RF_n - GLOBAL_X_LF_n)*0.0/0.4) + I_ZMP_CON_X*1.)*ZMP_FeedBack_ONOFF;
        ...
        CONT_X = GLOBAL_X_LIPM_n   +(- 0.001*Del_PC_X_DSP_XZMP_CON*G_DSP_X*(1 - fabs(GLOBAL_X_RF_n - GLOBAL_X_LF_n)*0.0/0.4) + I_ZMP_CON_X*1.)*ZMP_FeedBack_ONOFF;// +zmpONOFF*Kaj_x;
//    CONT_X = GLOBAL_X_LIPM_n   +(- 0.001*Del_PC_X_DSP_XZMP_CON*G_DSP_X*(1 - fabs(GLOBAL_X_RF_n - GLOBAL_X_LF_n)*0.3/0.4)- 0.001*Del_PC_X_SSP_XZMP_CON*0 + I_ZMP_CON_X*1.)*ZMP_FeedBack_ONOFF;
    ...
    else
    {
        ...
        CONT_Y = (GLOBAL_Y_LIPM_n)*1.0+ (-0.001*Del_PC_Y_DSP_YZMP_CON -0.001*Del_PC_Y_SSP_YZMP_CON*0+ I_ZMP_CON_Y*1.)*ZMP_FeedBack_ONOFF;// ;//;// + (GLOBAL_Y_LIPM_n)*(1-U_Gain) ;//- 0.001*Del_PC_Y_DSP_YZMP_CON*G_DSP_Y*1.0f;// + Add_COMTask_FootPrint[0][Ydir]+ Add_COMTask_FootPrint[1][Ydir];
    }
    ...
    for(int i=0; i<=LAR; i++)
    {
        ...
        if((fsm->walking_mode == TERRAIN_WALKING)||(fsm->walking_mode == LADDER_WALKING)||(fsm->walking_mode == TERRAIN_WALKING_ONE_STEP))
        {
            if(fabs(fsm->FootUpInfos[0][4])>0.06)
            {
                ...
                FWRefAngleCurrent[RAR] = WBIK_Q[RAR+7]*R2D*(1 + fsm->AddJointInfos[0][JRAR]*0./5.*Sagging_Comp_ONOFF + fsm->AddJointInfos[0][JLAR]*0./5.*Sagging_Comp_ONOFF) - fsm->AddJointInfos[0][JRAR]*(00. +0.0)*Sagging_Comp_ONOFF + AnkleRollAngle[HUBO_RIGHT]*Ankle_Moment_FeedBack_ONOFF+ GLOBAL_Xori_RF_n*Gyro_Ankle_FeedBack_ONOFF;
                FWRefAngleCurrent[LAR] = WBIK_Q[LAR+7]*R2D*(1 + fsm->AddJointInfos[0][JRAR]*0./5.*Sagging_Comp_ONOFF + fsm->AddJointInfos[0][JLAR]*0./5.*Sagging_Comp_ONOFF) + fsm->AddJointInfos[0][JLAR]*(0. +0.0)*Sagging_Comp_ONOFF + AnkleRollAngle[HUBO_LEFT]*Ankle_Moment_FeedBack_ONOFF+ GLOBAL_Xori_LF_n*Gyro_Ankle_FeedBack_ONOFF;;
            }
            else
            {
...
//                FWRefAngleCurrent[LAR] = WBIK_Q[LAR+7]*R2D*(1 + fsm->AddJointInfos[0][JRAR]*1/5. + fsm->AddJointInfos[0][JLAR]*1/5.) + fsm->AddJointInfos[0][JLAR]*(19. + (-( 0.13-GLOBAL_Y_LF)/0.04))+ GLOBAL_Xori_LF*Gyro_Ankle_FeedBack_ONOFF + AnkleRollAngle[HUBO_LEFT]*Ankle_Moment_FeedBack_ONOFF;
                //0.07,0.07
                FWRefAngleCurrent[RAR] = WBIK_Q[RAR+7]*R2D*(1 + fsm->AddJointInfos[0][JRAR]*0./5.*Sagging_Comp_ONOFF + fsm->AddJointInfos[0][JLAR]*0./5.*Sagging_Comp_ONOFF) - fsm->AddJointInfos[0][JRAR]*(0. +0.0)*Sagging_Comp_ONOFF + AnkleRollAngle[HUBO_RIGHT]*Ankle_Moment_FeedBack_ONOFF+ GLOBAL_Xori_RF_n*Gyro_Ankle_FeedBack_ONOFF;//
                FWRefAngleCurrent[LAR] = WBIK_Q[LAR+7]*R2D*(1 + fsm->AddJointInfos[0][JRAR]*0./5.*Sagging_Comp_ONOFF + fsm->AddJointInfos[0][JLAR]*0./5.*Sagging_Comp_ONOFF) + fsm->AddJointInfos[0][JLAR]*(0. +0.0)*Sagging_Comp_ONOFF + AnkleRollAngle[HUBO_LEFT]*Ankle_Moment_FeedBack_ONOFF+ GLOBAL_Xori_LF_n*Gyro_Ankle_FeedBack_ONOFF;//
            }
            ...
            //        printf("LAR = %f,RAR = %fLHR = %f,RHR = %f, FOG roll = %f\n",fsm->AddJointInfos[0][JLAR],fsm->AddJointInfos[0][JRAR],fsm->AddJointInfos[0][JLHR],fsm->AddJointInfos[0][JRHR],sharedData_FOG_Roll);
...
//            printf("fsm->FootUpInfos[0][4] = %f\n",fsm->FootUpInfos[0][4]);
            ...
            {
                FWRefAngleCurrent[RAR] = WBIK_Q[RAR+7]*R2D*(1 + fsm->AddJointInfos[0][JRAR]*0/5.*Sagging_Comp_ONOFF + fsm->AddJointInfos[0][JLAR]*0/5.*Sagging_Comp_ONOFF) - fsm->AddJointInfos[0][JRAR]*2.*Sagging_Comp_ONOFF + GLOBAL_Xori_RF_n*Gyro_Ankle_FeedBack_ONOFF + AnkleRollAngle[HUBO_RIGHT]*Ankle_Moment_FeedBack_ONOFF;
                FWRefAngleCurrent[LAR] = WBIK_Q[LAR+7]*R2D*(1 + fsm->AddJointInfos[0][JRAR]*0/5.*Sagging_Comp_ONOFF + fsm->AddJointInfos[0][JLAR]*0/5.*Sagging_Comp_ONOFF) + fsm->AddJointInfos[0][JLAR]*2.*Sagging_Comp_ONOFF + GLOBAL_Xori_LF_n*Gyro_Ankle_FeedBack_ONOFF + AnkleRollAngle[HUBO_LEFT]*Ankle_Moment_FeedBack_ONOFF;
            }
            else
            {
                FWRefAngleCurrent[RAR] = WBIK_Q[RAR+7]*R2D*(1 + fsm->AddJointInfos[0][JRAR]*0/5.*Sagging_Comp_ONOFF + fsm->AddJointInfos[0][JLAR]*0/5.*Sagging_Comp_ONOFF) - fsm->AddJointInfos[0][JRAR]*2.*Sagging_Comp_ONOFF+ GLOBAL_Xori_RF_n*Gyro_Ankle_FeedBack_ONOFF + AnkleRollAngle[HUBO_RIGHT]*Ankle_Moment_FeedBack_ONOFF;
...
//    joint->Joints[LSP]->RefAngleCurrent = FWRefAngleCurrent[i];
//    joint->Joints[RSP]->RefAngleCurrent = FWRefAngleCurrent[i];
...
//    static double EarlyLanding_FT = 100.;
    ...
    case STATE_DSP_RF:
        if((LF_FLAG==LF_ANKLE_LOCK)&&(once_flag[LEFT]==0))
        {
            ...
            {
                if((fabs(sharedData->FT[LAFT].Mx) >4.)||(fabs(sharedData->FT[LAFT].My) >4.)||(sharedData->FT[LAFT].Fz >20.))
                {
                    Free_LF_ANK();
                }
            }
    ...
    case STATE_SSP_RF:
        ...
        {
            if((fabs(sharedData->FT[RAFT].Mx) >4.)||(fabs(sharedData->FT[RAFT].My) >4.)||(sharedData->FT[RAFT].Fz >20.))
            {
    ...
    case STATE_SSP_LF:
        ...
        {
            if((fabs(sharedData->FT[LAFT].Mx) >4.)||(fabs(sharedData->FT[LAFT].My) >4.0)||(sharedData->FT[LAFT].Fz >20.))
            {
    ...
    case STATE_DSP_FINAL_RFLF:
        ...
        {
            if((fsm->walking_mode==LADDER_WALKING||fsm->walking_mode==TERRAIN_WALKING_ONE_STEP||fsm->walking_mode==TERRAIN_WALKING)&&((fabs(sharedData->FT[LAFT].Mx) >4.)||(fabs(sharedData->FT[LAFT].My) >4.0)||(sharedData->FT[LAFT].Fz >20.)))
            Free_LF_ANK();
        }
        if((RF_FLAG==RF_ANKLE_LOCK)&&(once_flag[RIGHT]==0)&&(pre_state == HUBO_RIGHT))
        {
            if((fsm->walking_mode==LADDER_WALKING||fsm->walking_mode==TERRAIN_WALKING_ONE_STEP||fsm->walking_mode==TERRAIN_WALKING)&&((fabs(sharedData->FT[RAFT].Mx) >4.)||(fabs(sharedData->FT[RAFT].My) >4.0)||(sharedData->FT[RAFT].Fz >20.)))
            Free_RF_ANK();
        }
    case STATE_FINISHED:
        break;
    ...
    {
        duty = 0.1*(0-sharedData->FT[LAFT].Mx);
        //printf("duty = %d\n",duty);
...
//        MCJointPWMCommand2chHR(JOINT_INFO[LAP].canch, JOINT_INFO[LAP].bno, 3, duty, 0, 0);
    ...
    {
        duty2 = 10*(0-sharedData->FT[RAFT].Mx);
        //printf("duty = %d\n",duty);
...
//    double totalMx;
    ...
    {
//        totalMx = sharedData->FT[RAFT].Mx+sharedData->FT[LAFT].Mx+(0.5f*kine_drc_hubo4.L_PEL2PEL+0.03)*sharedData->FT[LAFT].Fz+(-0.5f*kine_drc_hubo4.L_PEL2PEL+0.03)*sharedData->FT[RAFT].Fz;
        totalMy = sharedData->FT[RAFT].My+sharedData->FT[LAFT].My;

        X_ZMP_static = -totalMy/(sharedData->FT[LAFT].Fz+sharedData->FT[RAFT].Fz);
//        Y_ZMP_static = totalMx/(sharedData->FT[LAFT].Fz+sharedData->FT[RAFT].Fz);
    }
...
void get_zmp()
{
    ...
    if(sharedData->FT[RAFT].Fz + sharedData->FT[LAFT].Fz > 50.)
    {
        ...
        M_LF[0] =  sharedData->FT[LAFT].Mx*cos((fsm->LeftInfos[0][3] - (fsm->RightInfos[0][3] + fsm->LeftInfos[0][3])/2.)*D2R) - sharedData->FT[LAFT].My*sin((fsm->LeftInfos[0][3] - (fsm->RightInfos[0][3] + fsm->LeftInfos[0][3])/2.)*D2R);
        M_LF[1] =  sharedData->FT[LAFT].Mx*sin((fsm->LeftInfos[0][3] - (fsm->RightInfos[0][3] + fsm->LeftInfos[0][3])/2.)*D2R) + sharedData->FT[LAFT].My*cos((fsm->LeftInfos[0][3] - (fsm->RightInfos[0][3] + fsm->LeftInfos[0][3])/2.)*D2R);
        M_LF[2] = 0;

        M_RF[0] =  sharedData->FT[RAFT].Mx*cos((fsm->RightInfos[0][3] - (fsm->RightInfos[0][3] + fsm->LeftInfos[0][3])/2.)*D2R) - sharedData->FT[RAFT].My*sin((fsm->RightInfos[0][3] - (fsm->RightInfos[0][3] + fsm->LeftInfos[0][3])/2.)*D2R);
        M_RF[1] =  sharedData->FT[RAFT].Mx*sin((fsm->RightInfos[0][3] - (fsm->RightInfos[0][3] + fsm->LeftInfos[0][3])/2.)*D2R) + sharedData->FT[RAFT].My*cos((fsm->RightInfos[0][3] - (fsm->RightInfos[0][3] + fsm->LeftInfos[0][3])/2.)*D2R);
        M_RF[2] = 0;
...
//    const double Og[2] = {3.43082537995055,0.723663240519607};
    ...
    const double A[2][2] = {{0.000000000000, 1.000000000000}, {-246.7401,-1.0406}};
    const double B[2] = {0,423.4882};
    const double C[2] = {10.2884,0.0408};
    const double D = -16.5942;
    const double Kg[2] = {-0.5212,0.0212};
    const double Og[2] = {2.8235,-2.1929};
...
//            JW_Data_Debug[3][sJW_Data_Debug_Index] = (double)((unsigned char)(sharedData->TemperatureVoltage[23]))/4.0;//pv_Index;//Pel_Yaw;//HUBO2_ForceInitCon[RIGHT][Xdir];//des_pCOM_3x1_LPF[Zdir];//des_qPEL_4x1[0];//fsm->AddJointInfos[0][JLHR];//theta_dd;//GLOBAL_Yori_LF;//Dif_enc_LAR*R2D;//fw_LF_3x1[0];//fsm->RightInfos[0][0];
            ...
            JW_Data_Debug[11][sJW_Data_Debug_Index] = sharedData->FT[RAFT].Pitch;
            JW_Data_Debug[12][sJW_Data_Debug_Index] = sharedData->FT[RAFT].Roll;//GLOBAL_Z_RF;
...
//            JW_Data_Debug[49][sJW_Data_Debug_Index] = sharedData_FOG_Roll;//COMPEN_RHR;
//            JW_Data_Debug[50][sJW_Data_Debug_Index] = sharedData->FOG.Pitch;//COMPEN_LHR;

//            JW_Data_Debug[51][sJW_Data_Debug_Index] = sharedData_FOG_RollVel;
            JW_Data_Debug[52][sJW_Data_Debug_Index] = sharedData->IMU[0].PitchVel;
...
//        JW_Data_Debug2[61][sJW_Data_Debug_Index2] = sharedData->FT[RAFT].Fz;
//        JW_Data_Debug2[62][sJW_Data_Debug_Index2] = sharedData->FT[RAFT].Mx;
//        JW_Data_Debug2[63][sJW_Data_Debug_Index2] = sharedData->FT[RAFT].My;

//        JW_Data_Debug2[64][sJW_Data_Debug_Index2] = sharedData->FT[LAFT].Fz;
//        JW_Data_Debug2[65][sJW_Data_Debug_Index2] = sharedData->FT[LAFT].Mx;
//        JW_Data_Debug2[66][sJW_Data_Debug_Index2] = sharedData->FT[LAFT].My;
...
////        JW_Data_Debug2[76][sJW_Data_Debug_Index2] = sharedData_FOG_RollVel;//GLOBAL_Z_RF;
...
////        JW_Data_Debug2[79][sJW_Data_Debug_Index2] = sharedData->IMU[0].PitchVel;//GLOBAL_Z_LF;
...
////        JW_Data_Debug2[76][sJW_Data_Debug_Index2] = _temp_debug_data[7];//sharedData->IMU[0].RollVel;//GLOBAL_Z_RF;
...
//        JW_Data_Debug2[79][sJW_Data_Debug_Index2] = _temp_debug_data[10];//sharedData->IMU[0].PitchVel;//GLOBAL_Z_LF;
...
//            JW_Data_Debug[52][sJW_Data_Debug_Index] = sharedData->FOG.Roll;
...
//                //cout<<"======================================================"<<endl;
        ...
        //------------------------------------USER COMMAND-------------------------------------//
        checkfsm->walking_mode = sharedData->COMMAND[PODO_NO].USER_PARA_INT[9];
        ...
        for(int i=0; i<sharedData->COMMAND[PODO_NO].USER_PARA_INT[5]; i++)
        {
            ...
            task.COMz[1]    = 0;//sharedData->COMMAND[PODO_NO].USER_PARA_DOUBLE[i*14+13];
...
//    RDRoll = RightRollTorqueControl(RDesTorqueX,sharedData->FT[RAFT].Mx, fsm->RightInfos[0][4]*D2R);
...
//    LDRoll = LeftRollTorqueControl(LDesTorqueX,sharedData->FT[LAFT].Mx,fsm->LeftInfos[0][4]*D2R);

//    RDRoll = RX_TC(state,state2,state3,GLOBAL_ZMP_REF_X_n,GLOBAL_ZMP_REF_Y_n,X_ZMP_n/1000.0,Y_ZMP_n/1000.0,des_pLF_3x1[0],des_pRF_3x1[0],des_pLF_3x1[1],des_pRF_3x1[1],sharedData->FT[RAFT].Mx,fsm->RightInfos[0][4]*D2R);
//    LDRoll = LX_TC(state,state2,state3,GLOBAL_ZMP_REF_X_n,GLOBAL_ZMP_REF_Y_n,X_ZMP_n/1000.0,Y_ZMP_n/1000.0,des_pLF_3x1[0],des_pRF_3x1[0],des_pLF_3x1[1],des_pRF_3x1[1],sharedData->FT[LAFT].Mx,fsm->LeftInfos[0][4]*D2R);

//    RDPitch = RY_TC(state,state2,state3,GLOBAL_ZMP_REF_X_n,GLOBAL_ZMP_REF_Y_n,X_ZMP_n/1000.0,Y_ZMP_n/1000.0,des_pLF_3x1[0],des_pRF_3x1[0],des_pLF_3x1[1],des_pRF_3x1[1],sharedData->FT[RAFT].Mx,fsm->RightInfos[0][5]*D2R);
//    LDPitch = LY_TC(state,state2,state3,GLOBAL_ZMP_REF_X_n,GLOBAL_ZMP_REF_Y_n,X_ZMP_n/1000.0,Y_ZMP_n/1000.0,des_pLF_3x1[0],des_pRF_3x1[0],des_pLF_3x1[1],des_pRF_3x1[1],sharedData->FT[LAFT].Mx,fsm->LeftInfos[0][5]*D2R);
    ...
    Zctrl = FootForceControl(RDF,LDF,sharedData->FT[RAFT].Fz,sharedData->FT[LAFT].Fz,state,0.0);

//    LZctrl = LFootForceControl(RDF,LDF,sharedData->FT[RAFT].Fz,sharedData->FT[LAFT].Fz,state,fsm->LeftInfos[0][2]);

//    RZctrl = RFootForceControl(RDF,LDF,sharedData->FT[RAFT].Fz,sharedData->FT[LAFT].Fz,state,fsm->RightInfos[0][2]);


//    LZctrl = LFootForceControl(RDF,LDF,sharedData->FT[RAFT].Fz,sharedData->FT[LAFT].Fz,state,0.0);

//    RZctrl = RFootForceControl(RDF,LDF,sharedData->FT[RAFT].Fz,sharedData->FT[LAFT].Fz,state,0.0);
}
...
int pyeongchang_foot_print()
{
    printf("+++++++++Pyeongchang_foot_print+++++++++++\n");
    int num = 11;
    int foot = FOOT_RIGHT;

//    foot = pyeongchang_foot_print_1st_cwrot(num, foot);
//    foot = pyeongchang_foot_print_2nd_cwrot(num, foot);
//    foot = pyeongchang_foot_print_1st_cwrot(num, foot);
//    foot = pyeongchang_foot_print_2nd_cwrot(num, foot);
//    foot = pyeongchang_foot_print_1st_cwrot(num, foot);
//    foot = pyeongchang_foot_print_2nd_cwrot(num, foot);
...
//    foot = pyeongchang_foot_print_2nd_cwrot(num, foot);
    num = 10;
    foot = pyeongchang_foot_print_0st(num, foot);
    num = 12;
    foot = pyeongchang_foot_print_2nd(num, foot);
    num = 12;
    foot = pyeongchang_foot_print_1st(num, foot);
    num = 12;
    foot = pyeongchang_foot_print_2nd(num, foot);
    num = 12;
    foot = pyeongchang_foot_print_1st(num, foot);
...
// --------------------------------------------------------------------------------------------- //
float tune_length = -0.006;
float tune_angle = -1.7;
int pyeongchang_foot_print_0st(int _num,int _foot)
{
    DSPTask task;
    double length = tune_length;
    int pos;
    for(int i=0; i<_num+1; i++)
    {
        ...
        if(_foot == FOOT_RIGHT)
        {
            ...
            task.Right[0] = fsm->DSPScheduler[pos].Left[0] + L_PEL2PEL*sin((fsm->DSPScheduler[pos].LYaw + fsm->DSPScheduler[pos].RYaw)/2.*D2R) + length*0.8*cos((fsm->DSPScheduler[pos].LYaw + fsm->DSPScheduler[pos].RYaw)/2.*D2R);
            task.Right[1] = fsm->DSPScheduler[pos].Left[1] - L_PEL2PEL*cos((fsm->DSPScheduler[pos].LYaw + fsm->DSPScheduler[pos].RYaw)/2.*D2R) + length*sin((fsm->DSPScheduler[pos].LYaw + fsm->DSPScheduler[pos].RYaw)/2.*D2R);
            task.Right[2] = fsm->DSPScheduler[pos].Left[2];
            ...
            task.LRoll = fsm->DSPScheduler[pos].LRoll;
            task.LPitch = fsm->DSPScheduler[pos].LPitch;
            if(pre_foot == FOOT_LEFT)
                task.RYaw = fsm->DSPScheduler[pos].RYaw + tune_angle;
            else
                task.RYaw = fsm->DSPScheduler[pos].RYaw;
            task.RRoll = 0;
            task.RPitch = 0;
        ...
        else
        {
            ...
            if(pre_foot == FOOT_RIGHT)
                task.LYaw = fsm->DSPScheduler[pos].LYaw + tune_angle;
            else
                task.LYaw = fsm->DSPScheduler[pos].LYaw;
            task.LRoll = 0;
            task.LPitch = 0;
