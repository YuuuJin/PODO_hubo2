...
public:
    ...
    explicit ControllerControlClass(pRBCORE_SHM _shm, int _podoNum){
        Shm = _shm;
        PODONum = _podoNum;
        AddJoints = JointVector(NO_OF_JOINTS);
        for(int i=0; i<NO_OF_JOINTS; i++)
            AddJoints[i] = new JointClass(i, MC_GetID(i), MC_GetCH(i));
    }

    JointVector		AddJoints;

    double  GetJointRefAngle(const int n)						{return AddJoints[n]->GetRefAngleCurrent();}
    void    SetJointRefAngle(const int n, const double _ref)	{AddJoints[n]->SetRefAngleCurrent(_ref);}

    void    SetZero(){
        for(int i=0; i<NO_OF_JOINTS; i++){
            AddJoints[i]->RefAngleCurrent = 0.0;
        }
    }

    char	SetMoveJoint(const int _jnum, const double _angle, const double _msTime, const unsigned int _mode){
        return AddJoints[_jnum]->SetMoveJoint(_angle, _msTime, _mode);
    }
    char	MoveJoint(const int _jnum){
        return AddJoints[_jnum]->MoveJoint();
    }
    void    MoveAllJoint(){
        for(int i=0; i<NO_OF_JOINTS; i++){
            MoveJoint(i);
...
public:
    ...
    void    JointUpdate(){
        ...
        for(int i=0; i<NO_OF_JOINTS; i++){
            mcId = Joints[i]->MCId;
            mcCh = Joints[i]->MCCh;
            temp = _cCon->AddJoints[i]->GetRefAngleCurrent();
//            if(temp > 10.0)     temp = 10.0;
//            if(temp < -10.0)    temp = -10.0;

            Shm->JointReference[PODONum][mcId][mcCh] = Joints[i]->GetRefAngleCurrent() + temp;
    ...
    void    RefreshToCurrentReference(){
        ...
        for(int i=0; i<NO_OF_JOINTS; i++){
            ...
            temp2 = Shm->ENCODER[mcId][mcCh].CurrentReference-temp;
            Joints[i]->SetRefAngleCurrent(temp2);
