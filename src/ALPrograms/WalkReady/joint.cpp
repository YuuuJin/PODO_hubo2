...
unsigned char JointClass::SetMoveJoint(const double _angle, const double _msTime, const unsigned int _mode){
	if(_msTime <= 0) { std::cout << ">>> Goal time must be grater than zero..!!(SetMoveJoint)" << std::endl; return ERR_GOAL_TIME; }
	...
	case MOVE_ABSOLUTE:	// absolute mode
		...
		break;
	default:
		std::cout << ">>> Wrong reference mode(RBSetMoveJoint)..!!" << std::endl;
