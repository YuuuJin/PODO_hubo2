...
unsigned long WalkingModeCount=0;
unsigned int Walking_Start_counter = 0;
unsigned int Walking_Stop_counter = 0;
...
bool Walking_Start_flag = false;
bool Walking_Stop_flag = false;
...
bool Continuous_walking_mode =false;

unsigned int Walking_State = NOT_WALKING;
...
//    AlunumWMupperbody=PODOALDialog::GetALNumFromALName("WMupperbody_AL");
    ...
    PODO_DATA.UserM2G.Walking_state = false;
}
...
void JoyStickDialog::DisplayUpdate()
{
    ...
    if(JoysticWalkingFlag == true){
        ...
        if(Continuous_walking_mode == false && Walking_State == true && PODO_DATA.UserM2G.step_phase >= 0
                && PODO_DATA.UserM2G.Walking_state == true){
            USER_COMMAND cmd;
...
//            cmd.COMMAND_DATA.USER_PARA_CHAR[3]=(char)JOY_B;
            ...
            cmd.COMMAND_DATA.USER_PARA_DOUBLE[15] = 0.7;
        ...
        // jog push --> walking
        if(fabs(JOY_LJOG_RL) > 0 || fabs(JOY_LJOG_UD) > 0 || fabs(JOY_RJOG_RL) > 0 || fabs(JOY_RJOG_UD) > 0 ){
            if(Walking_State == NOT_WALKING && Continuous_walking_mode == false && PODO_DATA.UserM2G.Walking_state == false){
                Walking_Start_counter++;
                std::cout<<"start_count"<<std::endl;
            }
            else{
                Walking_Start_counter = 0;
        ...
        // jog 0 --> stop
        if(fabs(JOY_LJOG_RL) == 0 && fabs(JOY_LJOG_UD) == 0 && fabs(JOY_RJOG_RL) == 0 && fabs(JOY_RJOG_UD) == 0){
            if(Walking_State == WALKING && Continuous_walking_mode == false && PODO_DATA.UserM2G.Walking_state == true){
                Walking_Stop_counter++;
                std::cout<<"stop_count"<<std::endl;
            }
            else{
                Walking_Stop_counter = 0;
                Walking_Stop_flag = false;
            }
        }
        else{
            Walking_Stop_counter = 0;
        ...
        if(Walking_Start_counter > 5 && Walking_Start_flag == false && Walking_State == NOT_WALKING && PODO_DATA.UserM2G.Walking_state == false){
            ...
            cmd.COMMAND_DATA.USER_PARA_DOUBLE[0] = 0.7;
        ...
        if(Walking_Stop_counter > 5 && Walking_Stop_flag == false && Walking_State == WALKING && PODO_DATA.UserM2G.Walking_state == true){
            Walking_Stop_flag = true;
            Walking_State = NOT_WALKING;
            des_t_step = 0.7;
            USER_COMMAND cmd;
        ...
        if(JOY_A_counter > 10 && JOY_A_flag == false && PODO_DATA.UserM2G.Walking_state == false){
            // Walking Start
            Continuous_walking_mode = true;
            JOY_A_flag = true;
            des_t_step = 0.7;
            USER_COMMAND cmd;
            cmd.COMMAND_DATA.USER_COMMAND = HBWalking_PrevWalk;
        ...
        if(JOY_B_counter > 10 && JOY_B_flag == false && PODO_DATA.UserM2G.Walking_state == true){ // Walking terminate
            // Walking stop
            Continuous_walking_mode = false;
            JOY_B_flag = true;
//            on_MANUAL_BTN_JOY_WALKING_OFF_clicked();
        ...
        if(JOY_ARROW_DN_counter > 5 && JOY_ARROW_DN_flag == false){
            ...
            if(des_t_step <= 0.54) des_t_step = 0.54;
        ...
        if(JOY_Y_counter > 5 && JOY_Y_flag == false){  // IMU nulling
            ...
            cmd.COMMAND_DATA.USER_COMMAND = WALKREADY_WST_RESPOND;
            cmd.COMMAND_TARGET = AlnumWalkReady;
            pLAN->SendCommand(cmd);
        ...
        if(JOY_A == 0 && JOY_B == 0 && Continuous_walking_mode == true){
            USER_COMMAND cmd;
            cmd.COMMAND_DATA.USER_PARA_CHAR[0]=(char)JOY_X;
            cmd.COMMAND_DATA.USER_PARA_CHAR[1]=(char)JOY_Y;
            cmd.COMMAND_DATA.USER_PARA_CHAR[2]=(char)JOY_A;
...
//        pLAN->SendCommand(cmd);
    ...
    ui->LE_DEBUG->setText(str.sprintf("%d", PODO_DATA.UserM2G.valveMode));
}
...
void JoyStickDialog::on_BT_WALK_READY_clicked()
{
    USER_COMMAND cmd;
    cmd.COMMAND_DATA.USER_COMMAND = WALKREADY_GO_WALKREADYPOS;
...
void JoyStickDialog::on_BT_ONE_LEG_READY_clicked()
{
    USER_COMMAND cmd;
    cmd.COMMAND_DATA.USER_COMMAND = WALKREADY_GO_WALKREADYPOS;
...
void JoyStickDialog::on_BT_ONE_LEG_READY_2_clicked()
{
    USER_COMMAND cmd;
    cmd.COMMAND_DATA.USER_COMMAND = WALKREADY_GO_WALKREADYPOS;
...
void JoyStickDialog::on_BT_WALK_TEST_clicked() //position walking
{
    USER_COMMAND cmd;
...
void JoyStickDialog::on_BT_LOGGING_clicked()
{
    USER_COMMAND cmd;
...
void JoyStickDialog::on_BT_DATA_SAVE_clicked()
{
    USER_COMMAND cmd;
...
void JoyStickDialog::on_BT_TEST_STOP_clicked()
{
    USER_COMMAND cmd;
...
void JoyStickDialog::on_MANUAL_BTN_JOY_WALKING_ON_clicked()
{
...
void JoyStickDialog::on_MANUAL_BTN_JOY_WALKING_OFF_clicked()
{
...
void JoyStickDialog::on_BT_HOME_POS_clicked()
{
    USER_COMMAND cmd;
    cmd.COMMAND_TARGET = AlnumWalkReady;
...
void JoyStickDialog::on_BT_WALK_FORWARD_clicked()
{
    USER_COMMAND cmd;
    cmd.COMMAND_DATA.USER_COMMAND = HBWalking_PrevWalk;//HBWalking_WALK_TEST;
    cmd.COMMAND_DATA.USER_PARA_INT[0] = 3; // No of step
    cmd.COMMAND_DATA.USER_PARA_DOUBLE[0] = 0.7; //STep time
    cmd.COMMAND_DATA.USER_PARA_DOUBLE[1] = 0.2; // step stride
    cmd.COMMAND_TARGET = AlnumHBWalking;
    pLAN->SendCommand(cmd);
}

void JoyStickDialog::on_BT_WALK_BACKWARD_clicked()
{
    USER_COMMAND cmd;
    cmd.COMMAND_DATA.USER_COMMAND = HBWalking_PrevWalk;//HBWalking_WALK_TEST;
    cmd.COMMAND_DATA.USER_PARA_INT[0] = 3; // No of step
