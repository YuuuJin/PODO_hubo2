    ...
    ui(new Ui::GUIMainWindow)
{
    ...
    dialogOrigDemo = new OriginalDemoDialog(this);
    ui->MAIN_TAB->addTab((QWidget*)dialogOrigDemo, "Sci Demo");

    dialogJoystick = new JoyStickDialog(this);
    ui->MAIN_TAB->addTab((QWidget*)dialogJoystick, "Joystick");

//    ui->MAIN_TAB->setCurrentIndex(3);
//    dialogpyeongchang = new PyeongchangDialog(this);
//    ui->MAIN_TAB->addTab((QWidget*)dialogpyeongchang, "Pyeongchang");
...
void GUIMainWindow::SLOT_NEW_DATA(){
    ...
    // Power ======================
    if(PODO_DATA.CoreSHM.SP[0].Voltage < 48.0){
        err += "Voltage Low ";
    }
    if(PODO_DATA.CoreSHM.SP[0].Current > 8.0){
        err += "Current High ";
    }

    ui->LB_VOLTAGE->setText(QString().sprintf("%.1fV", PODO_DATA.CoreSHM.SP[0].Voltage));
    ui->LB_CURRENT->setText(QString().sprintf("%.1fA", PODO_DATA.CoreSHM.SP[0].Current));
    // ============================

    // MC Stat ====================
    for(int i=0; i<NO_OF_JOINTS; i++){
        mSTAT ms = PODO_DATA.CoreSHM.MCStatus[MC_GetID(i)][MC_GetCH(i)];
    ...
    if(PODO_DATA.CoreSHM.REF_Enabled)   ui->LE_REF_STAT->setStyleSheet("background-color: green");
    else                                ui->LE_REF_STAT->setStyleSheet("background-color: red");

    if(PODO_DATA.CoreSHM.ENC_Enabled)   ui->LE_ENC_STAT->setStyleSheet("background-color: green");
    else                                ui->LE_ENC_STAT->setStyleSheet("background-color: red");

    if(PODO_DATA.CoreSHM.SEN_Enabled)   ui->LE_SEN_STAT->setStyleSheet("background-color: green");
    else                                ui->LE_SEN_STAT->setStyleSheet("background-color: red");

    if(PODO_DATA.CoreSHM.CAN_Enabled)   ui->LE_CAN_STAT->setStyleSheet("background-color: green");
