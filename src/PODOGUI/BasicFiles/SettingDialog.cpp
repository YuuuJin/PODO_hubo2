...
enum _SET_JointSequentialNumber_0{
    S_RHY = 0, S_RHR, S_RHP, S_RKN, S_RAP, S_RAR, S_NUM_0
};
enum _SET_JointSequentialNumber_1{
    S_LHY = 0, S_LHR, S_LHP, S_LKN, S_LAP, S_LAR, S_NUM_1
};
enum _SET_JointSequentialNumber_2{
    S_RSP = 0, S_RSR, S_RSY, S_REB, S_RWY, S_RWP, S_NUM_2
};
enum _SET_JointSequentialNumber_3{
    S_LSP = 0, S_LSR, S_LSY, S_LEB, S_LWY, S_LWP, S_NUM_3
};
enum _SET_JointSequentialNumber_4{
    S_WST, S_NKY, S_NK1, S_NK2, S_NUM_4
};
enum _SET_JointSequentialNumber_5{
    S_RF1, S_RF2, S_RF3, S_RF4, S_RF5, S_NUM_5
};
enum _SET_JointSequentialNumber_6{
    S_LF1, S_LF2, S_LF3, S_LF4, S_LF5, S_NUM_6
};
...
void SettingDialog::UpdateSettings(){
    ...
    for(int i=0; i<NO_OF_JOINTS; i++){
        ...
        mSTAT stat = PODO_DATA.CoreSHM.MCStatus[mcId][mcCh];
        str = "";
        if(PODO_DATA.CoreSHM.ENCODER[mcId][mcCh].BoardConnection) str += "C  ";
        else                                                      str += "N  ";
        ...
        }else if(stat.b.RUN == 1 && i >= NKY){
            tw->item(row, 0)->setBackgroundColor(QColor(100, 255, 100));    // green    Home(0) Run(on)
        ...
        //tw->item(row, 2)->setText(QString().sprintf("%2d", (int)PODO_DATA.CoreSHM.MCTemperature[mcId]));
        tw->item(row, 2)->setText(QString().sprintf("%2d", (int)PODO_DATA.CoreSHM.MotorTemperature[mcId][mcCh]));
        //if(PODO_DATA.CoreSHM.MCTemperature[mcId] > 60)
        if(PODO_DATA.CoreSHM.MotorTemperature[mcId][mcCh] > 60)
...
void SettingDialog::on_BTN_MOVE_JOINT_clicked(){
    ...
    if(lastSelected == RF1 || lastSelected == LF1 ||
...
void SettingDialog::on_BTN_INIT_HAND_clicked(){
    USER_COMMAND cmd;
...
void SettingDialog::on_BTN_INIT_HEAD_clicked(){
    USER_COMMAND cmd;
    cmd.COMMAND_DATA.USER_COMMAND = DAEMON_INIT_FIND_HOME_HAND_HEAD;
