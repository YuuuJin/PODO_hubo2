...
inline float JointReference(int jnum){
    return PODO_DATA.CoreSHM.ENCODER[MC_GetID(jnum)][MC_GetCH(jnum)].CurrentReference;
}
inline float JointEncoder(int jnum){
    return PODO_DATA.CoreSHM.ENCODER[MC_GetID(jnum)][MC_GetCH(jnum)].CurrentPosition;
}
