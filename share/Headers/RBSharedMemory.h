...
typedef struct _RBCORE_SHM_
{
    ...
    mSTAT           MCStatus[MAX_MC][MOTOR_5CH];
    float           MCTemperature[MAX_MC];
    float           MotorTemperature[MAX_MC][MOTOR_2CH];

    ENCODER_SENSOR  ENCODER[MAX_MC][MOTOR_5CH];
    FT_SENSOR       FT[MAX_FT];
    ...
    float           JointReference[MAX_AL][MAX_MC][MOTOR_5CH];


    int             SYNC_SIGNAL[MAX_AL];
    int             ACK_SIGNAL[MAX_MC][MOTOR_5CH];

    int             MotionOwner[MAX_MC][MOTOR_5CH];
...
typedef enum _COMMAND_SET_{
    ...
    DAEMON_INIT_FIND_HOME_HAND_HEAD,
